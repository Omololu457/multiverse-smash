...
function buildBaseAttack({ fighter, name, startup, active, recovery, damage, rangeX, rangeY, hitstun, pushX, launchY, hitstop = 6 }) {
  ...
  return {
    name, damage, total, timer: total,
    activeStart: startup, activeEnd: startup + active,
    rangeX, rangeY, hitstun, hitstop, pushX, launchY, hasHit: false
  };
}
...
export function resolveProjectileHits(projectiles = [], p1, p2, hitEffects = []) {
  ...
  for (let i = projectiles.length - 1; i >= 0; i--) {
    ...
    for (const fighter of fighters) {
      ...
      const radius  = proj.radius || proj.size || 10;
      const projBox = {
        x: proj.x - radius,
        y: proj.y - radius,
        w: radius * 2,
