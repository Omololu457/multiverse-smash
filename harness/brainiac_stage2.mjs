// harness/brainiac_stage2.mjs — STAGE 2: Brainiac's 5 basic normals (repurposed tentacle art) + the
// crouch-tentacle variant. For light(J)/heavy(K)/upAttack(I)/air(J airborne)/downAir(S+J airborne):
//   (1) it resolves the correct brainiac_*_uniform sheet (no 128² fallback box)
//   (2) it CONNECTS on the adjacent dummy (p2 health drops)
// Plus the crouch-attack VARIANT: hold Down + light → brainiac_crouchtentacle_uniform (combat.js
// _setCrouchVariant, opt-in via animationData.crouchLight). Finally a data-level contract that every
// normal + the crouch variant is wired to a real brainiac sheet. down_air REUSES the air sheet (flag).
...
async function crop(name) {
  ...
  if (!r) { await page.screenshot({ path: path.join(OUT, `brainiac_s2_${name}.png`) }); return; }
  ...
  await page.screenshot({ path: path.join(OUT, `brainiac_s2_${name}_crop.png`), clip });
}
...
try {
  ...
  console.log("\n── ground normals (repurposed tentacle strikes) ──");
  const ground = [
    ["light", "j", "brainiac_light_uniform"],
    ["heavy", "k", "brainiac_heavy_uniform"],
    ["upAttack", "i", "brainiac_up_uniform"],
  ];
  ...
  await setupAdjacent(50);
  {
    ...
    const mv = await waitSheet("brainiac_air_uniform");
    check(`air: sprite → brainiac_air_uniform`, (mv.spriteSheet || "").includes("brainiac_air_uniform"), `action=${mv.action} sheet=${mv.spriteSheet}`);
    await crop("air");
  ...
  await setupAdjacent(34);
  {
    ...
    const mv = await waitSheet("brainiac_air_uniform");
    check(`downAir: sprite → brainiac_air_uniform (reuse)`, (mv.spriteSheet || "").includes("brainiac_air_uniform"), `action=${mv.action} sheet=${mv.spriteSheet}`);
    await crop("downAir");
  ...
  // ── CROUCH-ATTACK VARIANT: hold Down + light → brainiac_crouchtentacle_uniform ──
  console.log("\n── crouch-tentacle variant (Down+light) ──");
  let csSheet = "", csDmg = 0;
  for (let attempt = 0; attempt < 6 && !(csSheet.includes("brainiac_crouchtentacle_uniform") && csDmg > 0); attempt++) {
    await setupAdjacent(50);
    const hp0 = (await p2()).health;
    await page.keyboard.down("s"); await waitFrames(3);
    await page.keyboard.down("j"); await waitFrames(1); await page.keyboard.up("j");
    const mv = await waitSheet("brainiac_crouchtentacle_uniform", 12);
    if ((mv.spriteSheet || "").includes("brainiac_crouchtentacle_uniform")) { csSheet = mv.spriteSheet; await crop("crouchtentacle"); }
    await waitFrames(20);
    const hp1 = (await p2()).health; csDmg += Math.max(0, hp0 - hp1);
    await page.keyboard.up("s"); await waitGrounded(); await waitFrames(4);
  }
  check("Down+light swaps to brainiac_crouchtentacle_uniform (crouch variant)", csSheet.includes("brainiac_crouchtentacle_uniform"), `sheet=${csSheet}`);
  check("crouch-tentacle connects (dmg)", csDmg > 0, `dmg=${csDmg}`);

  // ── DATA-LEVEL contract: all 5 normals + crouch variant wired to real brainiac sheets ──
  console.log("\n── data contract ──");
  ...
  check("all 5 normals + crouch variant wired to real brainiac sheets", allWired, JSON.stringify(Object.fromEntries(keys.map(k => [k, (ad[k]?.sheet || "MISSING").split("/").pop()]))));
  ...
  console.log(`\n${FAIL === 0 ? "✅" : "❌"} Brainiac Stage 2: ${PASS} passed, ${FAIL} failed — shots in harness/shots/brainiac_s2_*_crop.png`);
