// harness/dark_knight_skins_shot.mjs — Batman / Dark Knight 11-skin batch (Default + 8 coordinated recolors +
...
// Screenshots → harness/shots/dark_knight_skin_<id>.png.
...
const IDS = [
  ["default", ""],
  ["darkKnightCrimsonKnight", "crimsonknight"],
  ["darkKnightVerdantGuardian", "verdantguardian"],
  ["darkKnightObsidianKnight", "obsidianknight"],
  ["darkKnightGoldenSentinel", "goldensentinel"],
  ["darkKnightAzureVigilante", "azurevigilante"],
  ["darkKnightVioletNightfall", "violetnightfall"],
  ["darkKnightEmberKnight", "emberknight"],
  ["darkKnightFrostboundSentinel", "frostboundsentinel"],
  ["darkKnightVoidSovereign", "voidsovereign"],
  ["darkKnightBatmanBeyond", "batmanbeyond"],
];
const SHEETS = ["air", "crescent", "crouch", "crouchlight", "dive", "dodge", "flail", "glide", "grapple", "heavy", "hurt", "idle", "knockdown", "light", "mechattack", "mechidle", "mechwire", "pistol", "rageidle", "ragetransform", "walk", "win"];

// ── STATIC on-disk sweep — every recolored sheet + portrait exists for each of the 10 tags ──
let missing = [];
for (const [, tag] of IDS) {
  if (!tag) continue;
  for (const s of SHEETS) { const p = path.join(ROOT, `dark_knight_${s}_uniform__${tag}.png`); if (!(fs.existsSync(p) && fs.statSync(p).size > 128)) missing.push(`${tag}:${s}`); }
  const pp = path.join(ROOT, `dark_knight_portrait__${tag}.png`); if (!(fs.existsSync(pp) && fs.statSync(pp).size > 128)) missing.push(`${tag}:portrait`);
}
...
await pg.goto(`${base}/index.html?harness=1&p1=dark_knight`, { waitUntil: "load" });
...
for (const [id, tag] of IDS) {
  ...
  await wf(6);
  const allSheets = [];
  for (const act of ["idle", "light", "heavy", "win"]) {
  ...
  await pg.screenshot({ path: path.join(OUT, `dark_knight_skin_${id}.png`), clip: { x: 360, y: 300, width: 340, height: 360 } });
  await force(null); await wf(1);
  const ok = allSheets.every(s => s.includes("dark_knight_") && (!tag || s.includes(`__${tag}`)));
...
await pg.evaluate(() => window.__harness.setSkin("p1", "darkKnightVoidSovereign"));
await force("idle"); await wf(14);
const vf = await pg.evaluate(() => window.__harness.p1());
check("Void Sovereign applied + renders void sheet", (vf.skinId === "darkKnightVoidSovereign") && (vf.spriteSheet || "").includes("__voidsovereign"), `skin=${vf.skinId}`);
await force(null);
