...
// against the transformation/cinematic systems built since the original sweep. Method (the proven
// __samDual pattern): patch CanvasRenderingContext2D.drawImage to count, PER rAF FRAME, how many times a
// fighter's BODY/idle sheet is drawn; track the max. A fighter's body drawn >1× in a single frame == the
// two-instances bug. Matchups use p2 = an unrelated char so a pattern uniquely counts ONE fighter (except
// Chrollo, whose copy legitimately shares the opponent's sheet → 2 is EXPECTED there, 3+ = bug).
