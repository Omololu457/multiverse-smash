// harness/flash_stage2_shots.mjs — STAGE 2: 5 normals + "Speed Rush" command chain (Down+Heavy rekka)
// with a mid-chain interrupt test, plus a no-projectile sweep. Captures screenshots of each connect.
...
async function shot(name) { await page.screenshot({ path: path.join(OUT, `flash_s2_${name}.png`) }); }
const seen = new Set();
...
console.log("\n── 5 normals ──");
for (const [name, key, gap, sheet] of [["light", "j", 60, "flash_light_uniform"], ["heavy", "k", 66, "flash_heavy_uniform"], ["up", "i", 60, "flash_up_uniform"]]) {
  await prep(gap);
  ...
  check(`${name} connects & sheet`, d > 0 && (a.spriteSheet || seen.has(`./${sheet}.png`)), `−${d.toFixed(0)}`);
}
// air
...
console.log("\n── Speed Rush command chain (Down+Heavy rekka) ──");
...
// re-tap Heavy during recovery — must NOT advance (no clean hit latched)
let rec = false; for (let i = 0; i < 40; i++) { const p = await record(); if (!p.attacking) break; if (p.attackPhase === "recovery") { rec = true; break; } await waitFrames(1); }
if (rec) await tapKey("k");
const afterMove = (await p1()).currentMove;
check("interrupt: opener fired", openMove === "rush1", `open=${openMove}`);
check("interrupt: whiff does NOT advance to rush2", afterMove !== "rush2", `after=${afterMove || "(ended)"}`);
...
console.log(`\nRESULT ${PASS} pass / ${FAIL} fail — shots in harness/shots/flash_s2_*.png`);
