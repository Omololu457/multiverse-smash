...
async function toRose() {
  // LADDER: base → SSG (quick tap) → Rose (hold-release). SSG is now a MANDATORY WAYPOINT.
  await page.evaluate(() => window.__harness.setEnergy(200));
  await page.keyboard.down("p"); await wf(1); await page.keyboard.up("p"); await wf(3);   // → SSG (snappy)
  await page.evaluate(() => window.__harness.setEnergy(200));
