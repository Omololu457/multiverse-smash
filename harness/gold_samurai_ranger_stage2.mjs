// harness/gold_samurai_ranger_stage2.mjs
// STAGE 2 evidence: Gold Samurai Ranger's 5 normals connect + correct sheets, the SINGLE grounded
// up-attack launcher (Gold has no merged tap/hold), and the Toji-Rekka command chain (samRekka1→2→Fin,
// cancel-on-hit) with a mid-chain interrupt (whiff breaks the string). Shots → harness/shots/gold_stage2_*.png.
...
async function shot(tag) { await page.screenshot({ path: path.join(OUT, `gold_stage2_${tag}.png`) }); }
...
try {
  ...
  for (const [name, action, key, gap, sheetTag, dmgMin] of [
    ["light", "light", "j", 46, "gold_slash_uniform", 15],
    ["heavy", "heavy", "k", 46, "gold_lunge_uniform", 28],
  ]) {
    await prep(gap);
  ...
  console.log("\n── single grounded up-attack launcher (Gold = one tier, not merged) ──");
  await prep(42);
  ...
  check("up-attack sheet = gold_rising_uniform", (seen.get("up") || upRec.spriteSheet || "").includes("gold_rising_uniform"), `action=${upRec.action} sheet=${seen.get("up") || upRec.spriteSheet}`);
  ...
  check("air resolves to gold_aerial_uniform", (seen.get("air") || "").includes("gold_aerial_uniform"), `action=${airRec.action} sheet=${seen.get("air")}`);
  ...
  check("down_air resolves to gold_aerial_uniform", (seen.get("down_air") || "").includes("gold_aerial_uniform"), `action=${daRec.action} sheet=${seen.get("down_air")}`);
  ...
  check("samRekkaFin sheet = gold_launcher_uniform", (seen.get("samRekkaFin") || "").includes("gold_launcher_uniform"), `sheet=${seen.get("samRekkaFin")}`);
  ...
  console.log(`\n${fail === 0 ? "✅" : "❌"} Gold Stage 2: ${pass} passed, ${fail} failed`);
