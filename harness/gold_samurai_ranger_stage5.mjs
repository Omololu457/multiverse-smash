...
// STAGE 5 evidence: Gold's "Barracuda Blade: Light Finale" ULTIMATE — TIER-SCALING freeze cinematic.
// Reuses Red's proven freeze architecture (samuraiUltCine) with Gold's own light-saber barrage art + a
// LIGHT (gold) FX palette. BASE form → base launcher art + base damage; MEGA form → mega launcher art +
// higher damage. Fires in BOTH states; art + damage differ. Shots → gold_stage5_{base,mega}_ult.png.
...
async function shot(tag) { await page.screenshot({ path: path.join(OUT, `gold_stage5_${tag}.png`) }); }
...
try {
  ...
  check("BASE: renders the base launcher (ultimate) strip", b.sheet.includes("samurai_ranger_gold_launcher_uniform"), `sheet=${b.sheet}`);
  ...
  check("MEGA: renders the MEGA launcher (ultimate) strip", m.sheet.includes("samurai_ranger_gold_mega_launcher_uniform"), `sheet=${m.sheet}`);
  ...
  console.log(`\n${fail === 0 ? "✅" : "❌"} Gold Stage 5: ${pass} passed, ${fail} failed`);
