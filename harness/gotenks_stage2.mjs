// harness/gotenks_stage2.mjs
// STAGE 2 evidence: Gotenks's 5 normals + crouchLight (guard from Stage 1). All CONFIRMED distinct SS-sheet art.
// (1) WIRING — every normal action's animationData points at a real reslice'd gotenks_ sheet (no box).
// (2) CONNECT — light/heavy/up land damage on P2 and render their sheet (all via GLOBAL_DAMAGE_SCALE ×0.60).
//     ★up has its OWN rising-kick launcher art (gotenks_up_uniform), not a heavy reuse.
...
// Screenshots → harness/shots/gotenks_stage2_*.png. See GOTENKS_ASSET_MAP.md.
...
async function shot(tag) { await page.screenshot({ path: path.join(OUT, `gotenks_stage2_${tag}.png`) }); }
...
try {
  ...
  console.log("\n── (1) wiring: every normal action → a real gotenks_ sheet (no box) ──");
  for (const [k, tag] of [
  ...
  for (const [name, key, tag] of [["light", "j", "gotenks_light_uniform"], ["heavy", "k", "gotenks_heavy_uniform"], ["up", "i", "gotenks_up_uniform"]]) {
    await prep(50);
  ...
  await page.keyboard.down("j"); let sawAir = false; for (let i = 0; i < 5; i++) { const a = await p1(); if ((a.spriteSheet || "").includes("gotenks_air_uniform")) sawAir = true; await waitFrames(2); } await page.keyboard.up("j");
  await shot("air"); check("air (airborne J) renders gotenks_air_uniform", sawAir, "");
  await waitGrounded();
  await prep(28); await page.evaluate(() => window.__harness.jumpP1?.()); await waitFrames(4);
  await page.keyboard.down("s"); await page.keyboard.down("j"); let sawDA = false; for (let i = 0; i < 5; i++) { const a = await p1(); if ((a.spriteSheet || "").includes("gotenks_air_uniform")) sawDA = true; await waitFrames(2); } await page.keyboard.up("j"); await page.keyboard.up("s");
  await shot("down_air"); check("down_air (airborne S+J) renders gotenks_air_uniform (reuse)", sawDA, "");
  await waitGrounded();
  ...
  let sawCr = false; for (let i = 0; i < 5; i++) { const a = await p1(); if ((a.spriteSheet || "").includes("gotenks_crouchlight_uniform")) sawCr = true; await waitFrames(2); }
  await page.keyboard.up("j"); await page.keyboard.up("s");
  await shot("crouchlight"); check("crouch+light renders gotenks_crouchlight_uniform", sawCr, "");
  await waitGrounded();

  console.log("\n── (3) guard: holding block renders the guard sheet ──");
  await prep(60);
  await page.keyboard.down(";"); let sawGuard = false; for (let i = 0; i < 6; i++) { const a = await p1(); if ((a.spriteSheet || "").includes("gotenks_guard_uniform")) sawGuard = true; await waitFrames(2); } await page.keyboard.up(";");
  await shot("guard"); check("guard renders gotenks_guard_uniform", sawGuard, "");
