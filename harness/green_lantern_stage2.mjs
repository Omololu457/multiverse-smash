// harness/green_lantern_stage2.mjs — STAGE 2: Green Lantern's 5 basic normals.
// For light(J)/heavy(K)/upAttack(I)/air(J airborne)/downAir(S+J airborne):
//   (1) it resolves the correct gl_*_uniform sheet (no 128² fallback box)
//   (2) it CONNECTS on the adjacent dummy (p2 health drops)
// Finally a data-level contract that every normal is wired to a real gl_ sheet. down_air REUSES the air
// sheet (flag). GL has NO crouch art → no crouch-attack variant (unlike Deathstroke).
...
async function crop(name) {
  ...
  if (!r) { await page.screenshot({ path: path.join(OUT, `green_lantern_s2_${name}.png`) }); return; }
  ...
  await page.screenshot({ path: path.join(OUT, `green_lantern_s2_${name}_crop.png`), clip });
}
...
try {
  ...
  console.log("\n── ground normals ──");
  const ground = [
    ["light", "j", "gl_light_uniform"],
    ["heavy", "k", "gl_heavy_uniform"],
    ["upAttack", "i", "gl_up_uniform"],
  ];
  ...
  await setupAdjacent(46);
  {
    ...
    const mv = await waitSheet("gl_air_uniform");
    check(`air: sprite → gl_air_uniform`, (mv.spriteSheet || "").includes("gl_air_uniform"), `action=${mv.action} sheet=${mv.spriteSheet}`);
    await crop("air");
  ...
  await setupAdjacent(30);
  {
    ...
    const mv = await waitSheet("gl_air_uniform");
    check(`downAir: sprite → gl_air_uniform (reuse)`, (mv.spriteSheet || "").includes("gl_air_uniform"), `action=${mv.action} sheet=${mv.spriteSheet}`);
    await crop("downAir");
  ...
  // ── DATA-LEVEL contract: all 5 normals wired to real gl_ sheets ──
  console.log("\n── data contract ──");
  const ad = await page.evaluate(() => window.__harness.charDef("green_lantern")?.animationData || {});
  const keys = ["light", "heavy", "up", "air", "down_air"];
  const allWired = keys.every(k => typeof ad[k]?.sheet === "string" && ad[k].sheet.includes("gl_"));
  check("all 5 normals wired to real gl_ sheets", allWired, JSON.stringify(Object.fromEntries(keys.map(k => [k, (ad[k]?.sheet || "MISSING").split("/").pop()]))));
  ...
  const ba = await page.evaluate(() => window.__harness.charDef("green_lantern")?.basic_attacks || {});
  check("upAttack is a launcher (knockbackY<0 + launch)", (ba.upAttack?.knockbackY || 0) < 0 && (ba.upAttack?.launch || 0) > 0, `kbY=${ba.upAttack?.knockbackY} launch=${ba.upAttack?.launch}`);

  check("no JS page errors", jsErrors.length === 0, jsErrors.slice(0, 3).join(" | "));
  console.log(`\n${FAIL === 0 ? "✅" : "❌"} Green Lantern Stage 2: ${PASS} passed, ${FAIL} failed — shots in harness/shots/green_lantern_s2_*_crop.png`);
