// harness/green_samurai_ranger.test.mjs
// CANONICAL full-kit test for the Green Samurai Ranger (Forest) — the single authoritative suite.
// Covers: registration/portrait/stats · 5 normals + single up-attack + Toji-Rekka chain + interrupt (base) ·
// Forest Spear slash-wave PROJECTILE (both tiers, tier-scaling) · Mega Mode FOREST-SYMBOL transformation
// (build→resolve tier-swap) + dual-render "two instances" guard · ≥3 moves on the Mega tier ·
// Ultimate TIER-SCALING (base art+dmg vs Mega art+dmg). Run: node harness/green_samurai_ranger.test.mjs
...
try {
  ...
  await page.evaluate(() => {
    ...
    if (!proto.__greenPatched) {
      const orig = proto.drawImage;
      proto.drawImage = function (img, ...r) { try { const s = (img && (img.currentSrc || img.src)) || ""; if (s.includes("samurai_ranger_forest_mega_idle_uniform")) window.__samDual.cur++; } catch (e) {} return orig.call(this, img, ...r); };
      proto.__greenPatched = true;
  ...
  const g = await p1();
  check("P1 = green_samurai_ranger", g.key === "green_samurai_ranger", `key=${g.key}`);
  check("renders as sprites", g.hasSpriteHandler, "");
  check("HP 1190 / EN 165 / scale 1.85", g.maxHealth === 1190 && g.maxEnergy === 165 && Math.abs(g.spriteScale - 1.85) < 0.01, `HP=${g.maxHealth} EN=${g.maxEnergy} scale=${g.spriteScale}`);
  check("portrait file exists on disk", fs.existsSync(path.join(ROOT, "samurai_ranger_forest_portrait.png")), "");

  section("base normals connect + correct sheets");
  for (const [name, key, gap, tag, dmgMin] of [["light", "j", 46, "forest_slash_uniform", 15], ["heavy", "k", 46, "forest_lunge_uniform", 28]]) {
    await prep(gap);
    const hp0 = (await p2()).health;
  ...
  check("up-attack (single) = forest_rising_uniform + launches", sh(up).includes("forest_rising_uniform") && (!(await p2()).grounded || (await p2()).vy < -1), `sheet=${sh(up)}`);
  ...
  check("air = forest_aerial_uniform", sh(air).includes("forest_aerial_uniform"), `sheet=${sh(air)}`);
  ...
  check("down_air = forest_aerial_uniform", sh(da).includes("forest_aerial_uniform"), `sheet=${sh(da)}`);
  ...
  // Re-tap Heavy REPEATEDLY across the recovery window — the connect-gated cancel only advances on a fresh
  // Heavy edge landing during recovery, a narrow window, so several attempts make the live advance reliable
  // (harmless extra edges outside recovery fire nothing). Wired-through is confirmed via rekkaNext too.
  await page.keyboard.down("d");
  await page.keyboard.down("k"); await waitFrames(2); await page.keyboard.up("k"); await sample(4);
  for (let i = 0; i < 5; i++) { await page.keyboard.down("k"); await waitFrames(2); await page.keyboard.up("k"); await sample(6); }
  ...
  section("Forest Spear special — slash-wave projectile (base)");
  await prep(240);
  ...
  let sawWave = false;
  for (let i = 0; i < 24; i++) { if ((await projs()).some(p => (p.name || "").includes("forest_spear_wave"))) sawWave = true; await waitFrames(1); }
  const baseWaveDmg = shp0 - (await p2()).health;
  check("Forest Spear casts (spear-thrust pose) + spawns wave + connects", !!cast && sawWave && baseWaveDmg > 0, `cast=${!!cast} wave=${sawWave} dmg=${baseWaveDmg}`);

  section("Mega Mode FOREST-SYMBOL transformation + tier-swap");
  await setEnergy(40);
  ...
  check("MEGA move 1 — idle = forest_mega_idle_uniform", sh(mega).includes("forest_mega_idle_uniform"), `sheet=${sh(mega)}`);
  ...
  section("Forest Spear tier-scaling (Mega wave out-damages base)");
  ...
  check("Mega Forest Spear = forest_mega_spear_cast pose + out-damages base wave", (megaCast || "").includes("forest_mega_spear_cast_uniform") && megaWaveDmg > baseWaveDmg, `mega=${megaWaveDmg} base=${baseWaveDmg} cast=${megaCast}`);

  section("Ultimate TIER-SCALING (base art+dmg vs Mega art+dmg)");
  await page.evaluate(() => window.__harness.boot()); await waitFrames(6);
  const bu = await fireUlt();
  check("BASE ult: activates + base launcher art + damage", bu.active && bu.sheet.includes("forest_launcher_uniform") && bu.mega === false && bu.dmg > 0, `sheet=${bu.sheet} mega=${bu.mega} dmg=${bu.dmg}`);
  await page.evaluate(() => window.__harness.boot()); await waitFrames(6);
  await toMega();
  const mu = await fireUlt();
  check("MEGA ult: activates + mega launcher art + higher damage", mu.active && mu.sheet.includes("forest_mega_launcher_uniform") && mu.mega === true && mu.dmg > bu.dmg, `sheet=${mu.sheet} mega=${mu.mega} dmg=${mu.dmg} (base ${bu.dmg})`);

  section("integrity");
  check("no JS page errors", jsErrors.length === 0, jsErrors.slice(0, 3).join(" | "));

  console.log(`\n${fail === 0 ? "✅" : "❌"} GREEN SAMURAI RANGER full-kit: ${pass} passed, ${fail} failed`);
