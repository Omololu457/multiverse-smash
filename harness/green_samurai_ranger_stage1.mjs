// harness/green_samurai_ranger_stage1.mjs
// STAGE 1 evidence: Green Samurai Ranger (Forest / "Mike") registration + base-tier movement/state.
// Screenshots harness/shots/green_stage1_*.png AND asserts the sprite gate / sheets / stats / scale.
...
async function shot(tag) { await page.screenshot({ path: path.join(OUT, `green_stage1_${tag}.png`) }); }
...
try {
  ...
  check("P1 is Green Samurai Ranger", g.key === "green_samurai_ranger", `key=${g.key}`);
  check("renders as sprites (hasSpriteHandler)", g.hasSpriteHandler, "");
  check("idle sheet = samurai_ranger_forest_idle_uniform", (g.spriteSheet || "").includes("samurai_ranger_forest_idle_uniform"), `sheet=${g.spriteSheet}`);
  check("spriteScale = 1.85", Math.abs((g.spriteScale || 0) - 1.85) < 0.01, `spriteScale=${g.spriteScale}`);
  check("maxHealth = 1190", g.maxHealth === 1190, `HP=${g.maxHealth}`);
  ...
  check("walk/run uses samurai_ranger_forest_run_uniform", ((seen.get("run") || seen.get("walk") || "")).includes("samurai_ranger_forest_run_uniform"), `action=${rn.action} sheet=${rn.spriteSheet}`);
  ...
  check("jump uses samurai_ranger_forest_jump_uniform", (jp.spriteSheet || "").includes("samurai_ranger_forest_jump_uniform"), `action=${jp.action} sheet=${jp.spriteSheet}`);
  await waitGrounded();
  ...
  check("guard uses samurai_ranger_forest_guard_uniform", (gd.spriteSheet || "").includes("samurai_ranger_forest_guard_uniform"), `action=${gd.action} isBlocking=${gd.isBlocking} sheet=${gd.spriteSheet}`);

  await page.evaluate(() => window.__harness.hurtP1(24)); await waitFrames(3); const h = await record(); await shot("hurt");
  check("hurt resolves to samurai_ranger_forest_hurt_uniform", h.action === "hurt" && (h.spriteSheet || "").includes("samurai_ranger_forest_hurt_uniform"), `action=${h.action} sheet=${h.spriteSheet}`);
  ...
  console.log(`\n${fail === 0 ? "✅" : "❌"} Green Stage 1: ${pass} passed, ${fail} failed`);
