// harness/green_samurai_ranger_stage3.mjs
// STAGE 3 evidence: Green Mega Mode = full Vegeta-style TIER-SWAP + FOREST-SYMBOL transformation cinematic.
//  • charge-release into Mega Mode (below-threshold guard), 木 forest-symbol morph → RESOLVE (art+dmg swap).
...
//  • tap-revert + auto-revert-on-empty. Reuses Red/Gold's proven tier-swap machinery with Green's own art.
...
async function shot(tag) { await page.screenshot({ path: path.join(OUT, `green_stage3_${tag}.png`) }); }
...
try {
  ...
  await page.evaluate(() => {
    ...
    if (!proto.__greenPatched) {
      ...
      proto.drawImage = function (img, ...rest) { try { const s = (img && (img.currentSrc || img.src)) || ""; if (s.includes("samurai_ranger_forest_mega_idle_uniform")) window.__samDual.cur++; } catch (e) {} return orig.call(this, img, ...rest); };
  ...
  console.log("\n── FOREST-SYMBOL transformation cinematic (木 leaf-glow → resolve) ──");
  await setEnergy(165);
  ...
  await waitFrames(8); await shot("transform_morph");   // mid-morph: forest symbol materialising over green leaf-glow
  ...
  check("MOVE 1 — idle = forest_mega_idle_uniform", sheet(mega).includes("samurai_ranger_forest_mega_idle_uniform"), `sheet=${sheet(mega)}`);
  ...
  check("MOVE 2 — light = forest_mega_slash_uniform", sheet(lightMega).includes("samurai_ranger_forest_mega_slash_uniform"), `sheet=${sheet(lightMega)}`);
  ...
  check("MOVE 3 — up = forest_mega_rising_uniform", sheet(upMega).includes("samurai_ranger_forest_mega_rising_uniform"), `sheet=${sheet(upMega)}`);
  ...
  check("MOVE 4 — guard = forest_mega_guard_uniform", sheet(guardMega).includes("samurai_ranger_forest_mega_guard_uniform"), `sheet=${sheet(guardMega)} isBlocking=${guardMega.isBlocking}`);
  ...
  const rev = await p1();
  check("quick tap reverts to base (art + form)", rev.currentForm !== "megaMode" && !rev.hasSkinAnim && sheet(rev).includes("samurai_ranger_forest_idle_uniform"), `form=${rev.currentForm} skinAnim=${rev.hasSkinAnim} sheet=${sheet(rev)}`);
  ...
  console.log(`\n${fail === 0 ? "✅" : "❌"} Green Stage 3: ${pass} passed, ${fail} failed`);
