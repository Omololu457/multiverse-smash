// harness/gwen_skins_shot.mjs — Gwen 11-skin batch (Default + 8 coordinated recolors +
...
// Screenshots → harness/shots/gwen_skin_<id>.png.
...
const IDS = [
  ["default", ""],
  ["gwenCrimsonMana", "crimsonmana"],
  ["gwenVerdantSpark", "verdantspark"],
  ["gwenObsidianAnodite", "obsidiananodite"],
  ["gwenGoldenAura", "goldenaura"],
  ["gwenAzureClassic", "azureclassic"],
  ["gwenVioletReign", "violetreign"],
  ["gwenFrostboundSpark", "frostboundspark"],
  ["gwenEmberMana", "embermana"],
  ["gwenVoidSovereign", "voidsovereign"],
  ["gwenAnodite", "anodite"],
];
const SHEETS = ["blade", "cast", "crescent", "crouch", "crouchlight", "dash", "guard", "heavy", "hurt", "idle", "jump", "knockdown", "light", "run", "up", "walk", "win"];

// ── STATIC on-disk sweep — every recolored sheet + portrait exists for each of the 10 tags ──
let missing = [];
for (const [, tag] of IDS) {
  if (!tag) continue;
  for (const s of SHEETS) { const p = path.join(ROOT, `gwen_${s}_uniform__${tag}.png`); if (!(fs.existsSync(p) && fs.statSync(p).size > 128)) missing.push(`${tag}:${s}`); }
  const pp = path.join(ROOT, `gwen_portrait__${tag}.png`); if (!(fs.existsSync(pp) && fs.statSync(pp).size > 128)) missing.push(`${tag}:portrait`);
}
...
await pg.goto(`${base}/index.html?harness=1&p1=gwen`, { waitUntil: "load" });
...
for (const [id, tag] of IDS) {
  ...
  await pg.screenshot({ path: path.join(OUT, `gwen_skin_${id}.png`), clip: { x: 360, y: 300, width: 340, height: 360 } });
  await force(null); await wf(1);
  const ok = allSheets.every(s => s.includes("gwen_") && (!tag || s.includes(`__${tag}`)));
...
await pg.evaluate(() => window.__harness.setSkin("p1", "gwenVoidSovereign"));
await force("idle"); await wf(14);
const vf = await pg.evaluate(() => window.__harness.p1());
check("Void Sovereign applied + renders void sheet", (vf.skinId === "gwenVoidSovereign") && (vf.spriteSheet || "").includes("__voidsovereign"), `skin=${vf.skinId}`);
await force(null);
