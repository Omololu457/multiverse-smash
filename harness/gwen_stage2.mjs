// harness/gwen_stage2.mjs — STAGE 2: Gwen's 5 basic normals + crouchLight.
// For light(J)/heavy(K)/upAttack(I)/crouchLight(S+J grounded)/air(J airborne)/downAir(S+J airborne):
//   (1) resolves the correct gwen_*_uniform sheet (no 128² fallback box)
//   (2) CONNECTS on the adjacent dummy (p2 health drops)
// Plus a data-level contract that every normal is wired to a real gwen sheet. air + down_air REUSE the
// light punch sheet (honest reuse — no dedicated aerial art). 4 REAL distinct poses on the sheet:
// straight-punch(light) / lunge-punch(heavy) / kick(up-launcher) / low crouch-punch(crouchLight). S3 = none.
...
async function crop(name) {
  ...
  if (!r) { await page.screenshot({ path: path.join(OUT, `gwen_s2_${name}.png`) }); return; }
  ...
  await page.screenshot({ path: path.join(OUT, `gwen_s2_${name}_crop.png`), clip });
}
...
try {
  ...
  console.log("\n── ground normals ──");
  const ground = [
    ["light", "j", "gwen_light_uniform"],
    ["heavy", "k", "gwen_heavy_uniform"],
    ["upAttack", "i", "gwen_up_uniform"],
  ];
  ...
  console.log("\n── crouch normal ──");
  {
    ...
    const mv = await waitSheet("gwen_crouchlight_uniform");
    check(`crouchLight: sprite → gwen_crouchlight_uniform`, (mv.spriteSheet || "").includes("gwen_crouchlight_uniform"), `action=${mv.action} sheet=${mv.spriteSheet}`);
  ...
  // ── AIR neutral: air (J while airborne) — REUSES the light punch sheet ──
  console.log("\n── air normals ──");
  await setupAdjacent(44);
  {
    const hp0 = (await p2()).health;
    ...
    const mv = await waitSheet("gwen_light_uniform");
    check(`air: sprite → gwen_light_uniform (reuse)`, (mv.spriteSheet || "").includes("gwen_light_uniform"), `action=${mv.action} sheet=${mv.spriteSheet}`);
    await crop("air");
  ...
  await setupAdjacent(30);
  {
    ...
    const mv = await waitSheet("gwen_light_uniform");
    check(`downAir: sprite → gwen_light_uniform (reuse)`, (mv.spriteSheet || "").includes("gwen_light_uniform"), `action=${mv.action} sheet=${mv.spriteSheet}`);
    await crop("downAir");
  ...
  // ── DATA-LEVEL contract: all normals wired to real gwen sheets ──
  console.log("\n── data contract ──");
  ...
  const allWired = keys.every(k => typeof ad[k]?.sheet === "string" && ad[k].sheet.includes("gwen"));
  check("all normals wired to real gwen sheets", allWired, JSON.stringify(Object.fromEntries(keys.map(k => [k, (ad[k]?.sheet || "MISSING").split("/").pop()]))));
  check("air + down_air REUSE light sheet (honest reuse)", ad.air?.sheet === ad.light?.sheet && ad.down_air?.sheet === ad.light?.sheet, `air=${(ad.air?.sheet||"").split("/").pop()} down_air=${(ad.down_air?.sheet||"").split("/").pop()} light=${(ad.light?.sheet||"").split("/").pop()}`);
  const bad = await page.evaluate(() => window.__harness.charDef("gwen")?.basic_attacks || {});
  ...
  console.log(`\n${FAIL === 0 ? "✅" : "❌"} Gwen Stage 2: ${PASS} passed, ${FAIL} failed — shots in harness/shots/gwen_s2_*_crop.png`);
