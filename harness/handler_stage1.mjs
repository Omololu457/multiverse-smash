// harness/handler_stage1.mjs
// STAGE 1 evidence: The Handler (Jujutsu Kaisen) registration + movement/state.
...
// handler_*_uniform.png sheets (HONEST reuses: run=walk, dash=walk, guard=idle, fall=jump, getup=hurt;
// crouch/jump/hurt/knockdown are their own strips), measures on-screen body height, and sweeps for the
// 128×128 procedural fallback box. Screenshots → harness/shots/handler_stage1_*.png.
...
async function boot() { await page.goto(`${base}/index.html?harness=1&p1=handler`, { waitUntil: "load" }); await page.waitForFunction(() => !!window.__harness, null, { timeout: 15000 }); await page.mouse.click(640, 360); }
async function waitFrames(n) { const s = (await state()).frame; await page.waitForFunction(([a, b]) => window.__harness.state().frame >= a + b, [s, n], { timeout: 15000, polling: 16 }); }
async function waitGrounded() { await page.waitForFunction(() => { const p = window.__harness.p1(); return p.grounded && Math.abs(p.vy) < 0.5; }, null, { timeout: 8000, polling: 16 }).catch(() => {}); }
async function shot(tag) { await page.screenshot({ path: path.join(OUT, `handler_stage1_${tag}.png`) }); }
...
try {
  ...
  check("idle sheet = handler_idle_uniform", (g.spriteSheet || "").includes("handler_idle_uniform"), `sheet=${g.spriteSheet}`);
  ...
  check("portrait asset exists on disk (handler_portrait.png)", fs.existsSync(path.join(ROOT, "handler_portrait.png")), "");
  ...
  // The Handler has NO dedicated run/dash/guard art (confirmed gap): run/dash REUSE walk, guard REUSES
  // idle (braced), fall REUSES jump's apex, getup REUSES hurt. crouch/jump/hurt/knockdown are their own strips.
  ...
  check("walk uses handler_walk_uniform", /handler_walk_uniform/.test(wk.spriteSheet || ""), `action=${wk.spriteAction} sheet=${wk.spriteSheet}`);

  await force("run"); await waitFrames(4); const rn = await p1(); await shot("run"); await force(null); await waitFrames(2);
  check("run → handler_walk_uniform (reuse)", (rn.spriteSheet || "").includes("handler_walk_uniform"), `sheet=${rn.spriteSheet}`);

  await force("dash"); await waitFrames(3); const dsh = await p1(); await shot("dash"); await force(null); await waitFrames(2);
  check("dash → handler_walk_uniform (reuse)", (dsh.spriteSheet || "").includes("handler_walk_uniform"), `sheet=${dsh.spriteSheet}`);

  await force("crouch"); await waitFrames(4); const cr = await p1(); await shot("crouch"); await force(null); await waitFrames(2);
  check("crouch → handler_crouch_uniform", (cr.spriteSheet || "").includes("handler_crouch_uniform"), `sheet=${cr.spriteSheet}`);
  ...
  check("jump uses handler_jump_uniform", (jp.spriteSheet || "").includes("handler_jump_uniform"), `action=${jp.spriteAction} sheet=${jp.spriteSheet}`);
  await waitGrounded();

  await force("fall"); await waitFrames(3); const fl = await p1(); await shot("fall"); await force(null); await waitFrames(2);
  check("fall → handler_jump_uniform (reuse)", (fl.spriteSheet || "").includes("handler_jump_uniform"), `sheet=${fl.spriteSheet}`);

  await force("guard"); await waitFrames(4); const bl = await p1(); await shot("guard"); await force(null); await waitFrames(2);
  check("guard → handler_idle_uniform (reuse)", (bl.spriteSheet || "").includes("handler_idle_uniform"), `action=${bl.spriteAction} sheet=${bl.spriteSheet}`);

  await force("hurt"); await waitFrames(4); const ht = await p1(); await shot("hurt"); await force(null); await waitFrames(2);
  check("hurt → handler_hurt_uniform", (ht.spriteSheet || "").includes("handler_hurt_uniform"), `sheet=${ht.spriteSheet}`);

  await force("knockdown"); await waitFrames(3); const kd = await p1(); await shot("knockdown"); await force(null); await waitFrames(2);
  check("knockdown → handler_knockdown_uniform", (kd.spriteSheet || "").includes("handler_knockdown_uniform"), `sheet=${kd.spriteSheet}`);

  await force("getup"); await waitFrames(3); const gu = await p1(); await shot("getup"); await force(null); await waitFrames(2);
  check("getup → handler_hurt_uniform (reuse)", (gu.spriteSheet || "").includes("handler_hurt_uniform"), `sheet=${gu.spriteSheet}`);
  ...
  for (const act of ["idle", "walk", "run", "dash", "crouch", "jump", "fall", "guard", "hurt", "knockdown", "getup"]) {
    ...
    if (!sh.includes("handler_")) boxHit.push(`${act}:${sh || "null"}`);
    await force(null); await waitFrames(1);
  }
  check("every movement action resolves a real handler_ sheet (no box)", boxHit.length === 0, boxHit.join(" | "));
