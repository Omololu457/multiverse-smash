// harness/hashirama_voice.test.mjs — Hashirama voice wiring (audio-only, JA).
// (1) every pooled clip exists on disk + pools randomize + no double-pooling.
// (2) live triggers fire the right pool via a playSfxFile spy: per-technique casts (mokutonArm/treeSummon/
//     woodGolem/gates) + intro + win.
...
const pool = name => page.evaluate(p => window.__harness.hashiramaVoicePool(p), name);
const spyLog = () => page.evaluate(() => (window.__harness.__sound._sfxSpy || []).filter(f => /^hashirama_voice_/.test(f)));
...
// hold a direction + tap special until a voice line fires
...
const POOLS = ["kunai","mokutonArm","treeSummon","woodGolem","gates","woodPunch","woodClone","sealingJutsu","intro","taunt","combatBark","hitReact","lowHealth","win"];
...
for (const p of POOLS) {
  ...
  const samples = await page.evaluate(pp => window.__harness.hashiramaVoicePick(pp, 300), p);
...
ok(total === 52, `52 clips across 14 pools (total=${total})`);
...
await castCheck("Fwd+Special (Mokuton Arm)", "d", "mokutonArm");   // facing right, "d"=ArrowRight→forward
await castCheck("Down+Special (Tree Summon)", "s", "treeSummon");
await castCheck("Up+Special (Wood Golem)", "w", "woodGolem");
await castCheck("Back+Special (Gracious Deity Gates)", "a", "gates");
