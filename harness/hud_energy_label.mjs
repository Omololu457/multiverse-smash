...
// (p1, p2, expectedP1Label, expectedP2Label, shotName)
const MATCHUPS = [
  ["goku",   "naruto", "Ki",   "Chakra", "HUD_energy_ki_vs_chakra"],       // Dragon Ball vs Naruto (the required pair)
  ["vegeta", "sasuke", "Ki",   "Chakra", "HUD_energy_vegeta_vs_sasuke"],   // second DB-vs-Naruto pair
  ["gojo",   "rick",   "Cursed Energy", "Bullshit Science Energy", "HUD_energy_jjk_vs_rick"], // confirm JJK + Rick untouched
  ["toji",   "gojo",   "ENERGY",        "Cursed Energy",           "HUD_energy_toji_heavenly"], // Toji → HEAVENLY RESTRICTION path (energyType none, JJK)
];

let pass = 0, fail = 0;
for (const [p1k, p2k, expP1, expP2, shot] of MATCHUPS) {
  ...
  hrOk ? pass++ : fail++;
  console.log(`${ok1 && ok2 && hrOk ? "✅" : "❌"} ${p1k} → "${labels.p1}"${st.hr1 ? " [HEAVENLY RESTRICTION]" : ""} (exp "${expP1}") | ${p2k} → "${labels.p2}"${st.hr2 ? " [HEAVENLY RESTRICTION]" : ""} (exp "${expP2}")`);
