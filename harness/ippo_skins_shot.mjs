// harness/ippo_skins_shot.mjs — Ippo Makunouchi 11-skin batch (Default + 8 coordinated recolors +
...
// Screenshots → harness/shots/ippo_skin_<id>.png.
...
const IDS = [
  ["default", ""],
  ["ippoCrimsonChallenger", "crimsonchallenger"],
  ["ippoVerdantContender", "verdantcontender"],
  ["ippoObsidianFighter", "obsidianfighter"],
  ["ippoGoldenChampion", "goldenchampion"],
  ["ippoAzureBoxer", "azureboxer"],
  ["ippoVioletContender", "violetcontender"],
  ["ippoFrostboundFighter", "frostboundfighter"],
  ["ippoEmberChallenger", "emberchallenger"],
  ["ippoVoidSovereign", "voidsovereign"],
  ["ippoChampionshipGold", "championshipgold"],
];
const SHEETS = ["air", "airhook", "body", "crouchlight", "dempsey_flurry", "dempsey_weave", "dodge", "failed", "fall", "gazelle", "getup", "guard", "heavy", "hook", "hurt", "idle", "idlelow", "jab1", "jab2", "jump", "knockdown", "light", "lose", "up", "upper", "walk", "win"];

// ── STATIC on-disk sweep — every recolored sheet + portrait exists for each of the 10 tags ──
let missing = [];
for (const [, tag] of IDS) {
  if (!tag) continue;
  for (const s of SHEETS) { const p = path.join(ROOT, `ippo_${s}_uniform__${tag}.png`); if (!(fs.existsSync(p) && fs.statSync(p).size > 128)) missing.push(`${tag}:${s}`); }
  const pp = path.join(ROOT, `ippo_portrait__${tag}.png`); if (!(fs.existsSync(pp) && fs.statSync(pp).size > 128)) missing.push(`${tag}:portrait`);
}
...
await pg.goto(`${base}/index.html?harness=1&p1=ippo`, { waitUntil: "load" });
...
for (const [id, tag] of IDS) {
  ...
  await pg.screenshot({ path: path.join(OUT, `ippo_skin_${id}.png`), clip: { x: 360, y: 300, width: 340, height: 360 } });
  await force(null); await wf(1);
  const ok = allSheets.every(s => s.includes("ippo_") && (!tag || s.includes(`__${tag}`)));
...
await pg.evaluate(() => window.__harness.setSkin("p1", "ippoVoidSovereign"));
await force("idle"); await wf(14);
const vf = await pg.evaluate(() => window.__harness.p1());
check("Void Sovereign applied + renders void sheet", (vf.skinId === "ippoVoidSovereign") && (vf.spriteSheet || "").includes("__voidsovereign"), `skin=${vf.skinId}`);
await force(null);
