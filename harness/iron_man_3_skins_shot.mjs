// harness/iron_man_3_skins_shot.mjs — Iron Man 3 11-skin batch (Default + 8 coordinated recolors +
...
// Screenshots → harness/shots/iron_man_3_skin_<id>.png.
...
const IDS = [
  ["default", ""],
  ["ironMan3CrimsonOverdrive", "crimsonoverdrive"], ["ironMan3VerdantCircuit", "verdantcircuit"],
  ["ironMan3ObsidianMark", "obsidianmark"], ["ironMan3GoldenCore", "goldencore"],
  ["ironMan3AzureRepulsor", "azurerepulsor"], ["ironMan3VioletExtremis", "violetextremis"],
  ["ironMan3FrostboundMark", "frostboundmark"], ["ironMan3EmberCore", "embercore"],
  ["ironMan3VoidSovereign", "voidsovereign"], ["ironMan3MarkFortyTwo", "markfortytwo"],
];
const SHEETS = ["idle","run","jump","air","down_air","crouch","crouchlight","hurt","knockdown","getup",
  "light","heavy","up","super_move","super_move_air","intro","win"];

// ── STATIC on-disk sweep — every recolored sheet + portrait exists for each of the 10 tags ──
let missing = [];
for (const [, tag] of IDS) {
  if (!tag) continue;
  for (const s of SHEETS) { const p = path.join(ROOT, `iron_man_3_${s}_uniform__${tag}.png`); if (!(fs.existsSync(p) && fs.statSync(p).size > 128)) missing.push(`${tag}:${s}`); }
  const pp = path.join(ROOT, `iron_man_3_portrait__${tag}.png`); if (!(fs.existsSync(pp) && fs.statSync(pp).size > 128)) missing.push(`${tag}:portrait`);
}
...
await pg.goto(`${base}/index.html?harness=1&p1=iron_man_3`, { waitUntil: "load" });
...
for (const [id, tag] of IDS) {
  ...
  await wf(6);
  const allSheets = [];
  for (const act of ["idle", "heavy", "ironMan3SuperMove", "win"]) {
  ...
  await pg.screenshot({ path: path.join(OUT, `iron_man_3_skin_${id}.png`), clip: { x: 360, y: 300, width: 340, height: 360 } });
  await force(null); await wf(1);
  const ok = allSheets.every(s => s.includes("iron_man_3_") && (!tag || s.includes(`__${tag}`)));
...
await pg.evaluate(() => window.__harness.setSkin("p1", "ironMan3VoidSovereign"));
await force("idle"); await wf(14);
const vf = await pg.evaluate(() => window.__harness.p1());
check("Void Sovereign applied + renders void sheet", (vf.skinId === "ironMan3VoidSovereign") && (vf.spriteSheet || "").includes("__voidsovereign"), `skin=${vf.skinId}`);
await force(null);
