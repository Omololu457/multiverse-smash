// harness/iron_man_stage1.mjs
// STAGE 1 evidence: Iron Man 1 (Marvel) registration + movement/state.
// Asserts the sprite gate (handler / idle sheet / stats / scale / energyType), exercises idle / walk /
// run / dash / crouch / jump / fall / guard / hurt / knockdown / getup / intro over the RE-SLICED
// iron_man_*_uniform.png sheets (HONEST reuses: run=walk, fall=jump; guard + dash are their OWN art),
// measures on-screen body height, and sweeps for the 128×128 procedural fallback box.
// Screenshots → harness/shots/iron_man_stage1_*.png.
...
async function boot() { await page.goto(`${base}/index.html?harness=1&p1=iron_man`, { waitUntil: "load" }); await page.waitForFunction(() => !!window.__harness, null, { timeout: 15000 }); await page.mouse.click(640, 360); }
async function waitFrames(n) { const s = (await state()).frame; await page.waitForFunction(([a, b]) => window.__harness.state().frame >= a + b, [s, n], { timeout: 15000, polling: 16 }); }
async function waitGrounded() { await page.waitForFunction(() => { const p = window.__harness.p1(); return p.grounded && Math.abs(p.vy) < 0.5; }, null, { timeout: 8000, polling: 16 }).catch(() => {}); }
async function shot(tag) { await page.screenshot({ path: path.join(OUT, `iron_man_stage1_${tag}.png`) }); }
...
try {
  ...
  check("portrait asset exists on disk (iron_man_portrait.png)", fs.existsSync(path.join(ROOT, "iron_man_portrait.png")), "");
  ...
  // Iron Man aliases run→walk (no dedicated sprint art) but has DEDICATED dash art.
  ...
  check("walk uses iron_man_walk_uniform", /iron_man_walk_uniform/.test(wk.spriteSheet || ""), `action=${wk.spriteAction} sheet=${wk.spriteSheet}`);
  await force("run"); await waitFrames(4); const rn = await p1(); await shot("run"); await force(null); await waitFrames(2);
  check("run → iron_man_walk_uniform (reuse)", (rn.spriteSheet || "").includes("iron_man_walk_uniform"), `sheet=${rn.spriteSheet}`);

  await force("dash"); await waitFrames(3); const ds = await p1(); await shot("dash"); await force(null); await waitFrames(2);
  check("dash → iron_man_dash_uniform (own art)", (ds.spriteSheet || "").includes("iron_man_dash_uniform"), `sheet=${ds.spriteSheet}`);

  await force("crouch"); await waitFrames(4); const cr = await p1(); await shot("crouch"); await force(null); await waitFrames(2);
  check("crouch → iron_man_crouch_uniform", (cr.spriteSheet || "").includes("iron_man_crouch_uniform"), `sheet=${cr.spriteSheet}`);
  ...
  check("jump uses iron_man_jump_uniform", (jp.spriteSheet || "").includes("iron_man_jump_uniform"), `action=${jp.spriteAction} sheet=${jp.spriteSheet}`);
  await waitGrounded();

  await force("fall"); await waitFrames(3); const fl = await p1(); await shot("fall"); await force(null); await waitFrames(2);
  check("fall → iron_man_jump_uniform (reuse)", (fl.spriteSheet || "").includes("iron_man_jump_uniform"), `sheet=${fl.spriteSheet}`);

  await force("guard"); await waitFrames(4); const bl = await p1(); await shot("guard"); await force(null); await waitFrames(2);
  check("guard → iron_man_guard_uniform (own real block art)", (bl.spriteSheet || "").includes("iron_man_guard_uniform"), `action=${bl.spriteAction} sheet=${bl.spriteSheet}`);

  await force("hurt"); await waitFrames(4); const ht = await p1(); await shot("hurt"); await force(null); await waitFrames(2);
  check("hurt → iron_man_hurt_uniform", (ht.spriteSheet || "").includes("iron_man_hurt_uniform"), `sheet=${ht.spriteSheet}`);

  await force("knockdown"); await waitFrames(3); const kd = await p1(); await shot("knockdown"); await force(null); await waitFrames(2);
  check("knockdown → iron_man_knockdown_uniform", (kd.spriteSheet || "").includes("iron_man_knockdown_uniform"), `sheet=${kd.spriteSheet}`);

  await force("getup"); await waitFrames(3); const gu = await p1(); await shot("getup"); await force(null); await waitFrames(2);
  check("getup → iron_man_getup_uniform", (gu.spriteSheet || "").includes("iron_man_getup_uniform"), `sheet=${gu.spriteSheet}`);

  await force("intro"); await waitFrames(3); const ntr = await p1(); await shot("intro"); await force(null); await waitFrames(2);
  check("intro → iron_man_intro_uniform (helmet-open reveal art)", (ntr.spriteSheet || "").includes("iron_man_intro_uniform"), `sheet=${ntr.spriteSheet}`);

  console.log("\n── fallback-box sweep (no MOVEMENT action renders the 128×128 procedural box) ──");
  const boxHit = [];
  for (const act of ["idle", "walk", "run", "dash", "crouch", "jump", "fall", "guard", "hurt", "knockdown", "getup", "intro"]) {
    await force(act); await waitFrames(3); const r = await p1();
    const sh = r.spriteSheet || "";
    if (!sh.includes("iron_man_")) boxHit.push(`${act}:${sh || "null"}`);
    await force(null); await waitFrames(1);
  }
  check("every movement action resolves a real iron_man_ sheet (no box)", boxHit.length === 0, boxHit.join(" | "));
