// harness/iron_man_stage2.mjs — STAGE 2: Iron Man's 5 basic normals + the crouch-thrust variant.
// For light(J)/heavy(K)/upAttack(I)/air(J airborne)/downAir(S+J airborne):
//   (1) it resolves the correct iron_man_*_uniform sheet (no 128² fallback box)
//   (2) it CONNECTS on the adjacent dummy (p2 health drops)
// Plus the crouch-attack VARIANT: hold Down + light → iron_man_crouchthrust_uniform (combat.js
// _setCrouchVariant, opt-in via animationData.crouchLight). Finally a data-level contract that every
// normal + the crouch variant is wired to a real iron_man sheet. down_air REUSES the air sheet (flag).
...
async function crop(name) {
  ...
  if (!r) { await page.screenshot({ path: path.join(OUT, `iron_man_s2_${name}.png`) }); return; }
  ...
  await page.screenshot({ path: path.join(OUT, `iron_man_s2_${name}_crop.png`), clip });
}
...
try {
  ...
  console.log("\n── ground normals ──");
  const ground = [
    ["light", "j", "iron_man_light_uniform"],
    ["heavy", "k", "iron_man_heavy_uniform"],
    ["upAttack", "i", "iron_man_up_uniform"],
  ];
  ...
  await setupAdjacent(46);
  {
    ...
    const mv = await waitSheet("iron_man_air_uniform");
    check(`air: sprite → iron_man_air_uniform`, (mv.spriteSheet || "").includes("iron_man_air_uniform"), `action=${mv.action} sheet=${mv.spriteSheet}`);
    await crop("air");
  ...
  await setupAdjacent(30);
  {
    ...
    const mv = await waitSheet("iron_man_air_uniform");
    check(`downAir: sprite → iron_man_air_uniform (reuse)`, (mv.spriteSheet || "").includes("iron_man_air_uniform"), `action=${mv.action} sheet=${mv.spriteSheet}`);
    await crop("downAir");
  ...
  // ── CROUCH-ATTACK VARIANT: hold Down + light → iron_man_crouchthrust_uniform ──
  console.log("\n── crouch-thrust variant (Down+light) ──");
  let csSheet = "", csDmg = 0;
  for (let attempt = 0; attempt < 6 && !(csSheet.includes("iron_man_crouchthrust_uniform") && csDmg > 0); attempt++) {
    await setupAdjacent(46);
    const hp0 = (await p2()).health;
    await page.keyboard.down("s"); await waitFrames(3);
    await page.keyboard.down("j"); await waitFrames(1); await page.keyboard.up("j");
    const mv = await waitSheet("iron_man_crouchthrust_uniform", 12);
    if ((mv.spriteSheet || "").includes("iron_man_crouchthrust_uniform")) { csSheet = mv.spriteSheet; await crop("crouchthrust"); }
    await waitFrames(20);
    const hp1 = (await p2()).health; csDmg += Math.max(0, hp0 - hp1);
    await page.keyboard.up("s"); await waitGrounded(); await waitFrames(4);
  }
  check("Down+light swaps to iron_man_crouchthrust_uniform (crouch variant)", csSheet.includes("iron_man_crouchthrust_uniform"), `sheet=${csSheet}`);
  check("crouch-thrust connects (dmg)", csDmg > 0, `dmg=${csDmg}`);

  // ── DATA-LEVEL contract: all 5 normals + crouch variant wired to real iron_man sheets ──
  console.log("\n── data contract ──");
  ...
  check("all 5 normals + crouch variant wired to real iron_man sheets", allWired, JSON.stringify(Object.fromEntries(keys.map(k => [k, (ad[k]?.sheet || "MISSING").split("/").pop()]))));
