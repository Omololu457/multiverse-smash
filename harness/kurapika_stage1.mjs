// harness/kurapika_stage1.mjs
// STAGE 1 evidence: Kurapika (Hunter x Hunter) registration + movement/state.
// Asserts the sprite gate (handler / idle sheet / stats / scale / nen energyType), exercises idle / walk /
// run / dash / jump / fall / guard / hurt / knockdown / win over the RE-SLICED + DESPECKLED
// kurapika_*_uniform.png sheets, measures on-screen body height, and sweeps for the 128×128 fallback box.
// Screenshots → harness/shots/kurapika_stage1_*.png.
...
async function boot() { await page.goto(`${base}/index.html?harness=1&p1=kurapika`, { waitUntil: "load" }); await page.waitForFunction(() => !!window.__harness, null, { timeout: 15000 }); await page.mouse.click(640, 360); }
async function waitFrames(n) { const s = (await state()).frame; await page.waitForFunction(([a, b]) => window.__harness.state().frame >= a + b, [s, n], { timeout: 15000, polling: 16 }); }
async function waitGrounded() { await page.waitForFunction(() => { const p = window.__harness.p1(); return p.grounded && Math.abs(p.vy) < 0.5; }, null, { timeout: 8000, polling: 16 }).catch(() => {}); }
async function shot(tag) { await page.screenshot({ path: path.join(OUT, `kurapika_stage1_${tag}.png`) }); }
...
try {
  ...
  const def = await page.evaluate(() => window.__harness.charDef("kurapika"));
  check("universe = hunter_x_hunter", def?.universe === "hunter_x_hunter", `universe=${def?.universe}`);
  check("energyType = nen", def?.traits?.energyType === "nen", `energyType=${def?.traits?.energyType}`);
  ...
  check("walk/run uses a kurapika walk/run sheet", /kurapika_(walk|run)_uniform/.test(rn.spriteSheet || ""), `action=${rn.spriteAction} sheet=${rn.spriteSheet}`);

  await force("dash"); await waitFrames(3); const ds = await p1(); await shot("dash"); await force(null); await waitFrames(2);
  check("dash → kurapika_dash_uniform", (ds.spriteSheet || "").includes("kurapika_dash_uniform"), `sheet=${ds.spriteSheet}`);
  ...
  check("jump uses kurapika_leap_uniform (reuse)", (jp.spriteSheet || "").includes("kurapika_leap_uniform"), `action=${jp.spriteAction} sheet=${jp.spriteSheet}`);
  await waitGrounded();

  await force("guard"); await waitFrames(4); const bl = await p1(); await shot("guard"); await force(null); await waitFrames(2);
  check("guard → kurapika_guard_uniform", (bl.spriteSheet || "").includes("kurapika_guard_uniform"), `action=${bl.spriteAction} sheet=${bl.spriteSheet}`);

  await force("hurt"); await waitFrames(4); const ht = await p1(); await shot("hurt"); await force(null); await waitFrames(2);
  check("hurt → kurapika_hurt_uniform", (ht.spriteSheet || "").includes("kurapika_hurt_uniform"), `sheet=${ht.spriteSheet}`);

  await force("knockdown"); await waitFrames(3); const kd = await p1(); await shot("knockdown"); await force(null); await waitFrames(2);
  check("knockdown → kurapika_knockdown_uniform", (kd.spriteSheet || "").includes("kurapika_knockdown_uniform"), `sheet=${kd.spriteSheet}`);

  await force("win"); await waitFrames(4); const wn = await p1(); await shot("win"); await force(null); await waitFrames(2);
  check("win resolves a kurapika sheet (idle placeholder)", (wn.spriteSheet || "").includes("kurapika_idle_uniform"), `sheet=${wn.spriteSheet}`);

  console.log("\n── fallback-box sweep (no action renders the 128×128 procedural box) ──");
  const boxHit = [];
  for (const act of ["idle", "walk", "run", "dash", "jump", "fall", "guard", "hurt", "knockdown", "win"]) {
    await force(act); await waitFrames(3); const r = await p1();
    const sh = r.spriteSheet || "";
    if (!sh.includes("kurapika_")) boxHit.push(`${act}:${sh || "null"}`);
    await force(null); await waitFrames(1);
  }
  check("every action resolves a real kurapika_ sheet (no box)", boxHit.length === 0, boxHit.join(" | "));
