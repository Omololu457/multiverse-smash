// harness/kurapika_stage2.mjs — STAGE 2: Kurapika's 5 basic normals + Fwd+Heavy windmill command-normal.
// For each of light(J)/heavy(K)/upAttack(I)/air(J airborne)/downAir(S+J airborne):
//   (1) resolves the correct kurapika_*_uniform sheet (no 128² fallback box)
//   (2) CONNECTS on the adjacent dummy (p2 health drops)
// Then the command normal: Fwd+Heavy → kurapikaWindmill (8f multi-hit spin) sets currentMove and connects.
// Finally a data-level contract. Shots → harness/shots/kurapika_s2_*_crop.png.
...
async function crop(name) {
  ...
  if (!r) { await page.screenshot({ path: path.join(OUT, `kurapika_s2_${name}.png`) }); return; }
  ...
  await page.screenshot({ path: path.join(OUT, `kurapika_s2_${name}_crop.png`), clip });
}
...
try {
  ...
  console.log("\n── ground normals ──");
  const ground = [
    ["light", "j", "kurapika_light_uniform"],
    ["heavy", "k", "kurapika_heavy_uniform"],
    ["upAttack", "i", "kurapika_up_uniform"],
  ];
  ...
  await setupAdjacent(46);
  {
    ...
    const mv = await waitSheet("kurapika_air_uniform");
    check(`air: sprite → kurapika_air_uniform`, (mv.spriteSheet || "").includes("kurapika_air_uniform"), `action=${mv.action} sheet=${mv.spriteSheet}`);
    await crop("air");
  ...
  await setupAdjacent(30);
  {
    ...
    const mv = await waitSheet("kurapika_air_uniform");
    check(`downAir: sprite → kurapika_air_uniform (reuse leap-slash)`, (mv.spriteSheet || "").includes("kurapika_air_uniform"), `action=${mv.action} sheet=${mv.spriteSheet}`);
    await crop("downAir");
  ...
  console.log("\n── Fwd+Heavy command normal (kurapikaWindmill — multi-hit spin) ──");
  let comboSheet = "", comboMove = "", comboDmg = 0;
  for (let attempt = 0; attempt < 8 && !(comboSheet.includes("kurapika_windmill_uniform") && comboMove === "kurapikaWindmill" && comboDmg > 0); attempt++) {
    ...
    const mv = await waitSheet("kurapika_windmill_uniform", 12);
    if (mv.currentMove === "kurapikaWindmill") comboMove = mv.currentMove;
    if ((mv.spriteSheet || "").includes("kurapika_windmill_uniform")) { comboSheet = mv.spriteSheet; await crop("cmdwindmill"); }
    await waitFrames(24);
    const hp1 = (await p2()).health; comboDmg += Math.max(0, hp0 - hp1);
    await page.keyboard.up("d"); await waitGrounded(); await waitFrames(4);
  }
  check("command normal fires kurapikaWindmill (currentMove)", comboMove === "kurapikaWindmill", `move=${comboMove}`);
  check("kurapikaWindmill → kurapika_windmill_uniform sprite", comboSheet.includes("kurapika_windmill_uniform"), `sheet=${comboSheet}`);
  check("kurapikaWindmill connects + MULTI-HIT (dmg > single windmill 22)", comboDmg > 22, `dmg=${comboDmg}`);

  console.log("\n── data contract ──");
  const ad = await page.evaluate(() => window.__harness.charDef("kurapika")?.animationData || {});
  const keys = ["light", "heavy", "up", "air", "down_air", "kurapikaWindmill"];
  const allWired = keys.every(k => typeof ad[k]?.sheet === "string" && ad[k].sheet.includes("kurapika"));
  check("all 5 normals + command normal wired to real kurapika sheets", allWired, JSON.stringify(Object.fromEntries(keys.map(k => [k, (ad[k]?.sheet || "MISSING").split("/").pop()]))));

  check("no JS page errors", jsErrors.length === 0, jsErrors.slice(0, 3).join(" | "));
  console.log(`\n${FAIL === 0 ? "✅" : "❌"} Kurapika Stage 2: ${PASS} passed, ${FAIL} failed — shots in harness/shots/kurapika_s2_*_crop.png`);
