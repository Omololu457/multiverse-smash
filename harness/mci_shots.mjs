// harness/mci_shots.mjs — REAL in-game evidence that "Spider-Man (MCI)" (spiderman_mci) is a
...
// Usage: node harness/mci_shots.mjs
...
// ── PART 1 — CHARACTER SELECT: MCI appears in the marvel grid; existing chars untouched ──
...
ok(Array.isArray(roster) && roster.includes("spiderman_mci"), "spiderman_mci is in the character-select grid");
...
await sleep(500);
await page.screenshot({ path: path.join(OUT, "mci_select.png") });
console.log("  wrote harness/shots/mci_select.png");

// ── PART 2 — LIVE MATCH: MCI spawns, renders, moves, attacks ──
console.log("── live match ──");
await page.goto(`${base}/index.html?harness=1&p1=spiderman_mci&p2=jason`, { waitUntil:"load" });
await page.waitForFunction(() => !!window.__harness, null, { timeout:15000 });
...
ok(p1 && p1.key === "spiderman_mci", `p1 is spiderman_mci in the live match (key=${p1?.key})`);
...
await sleep(300);
await page.screenshot({ path: path.join(OUT, "mci_idle.png") });   // idle pose
console.log("  wrote harness/shots/mci_idle.png");

// walk right
await page.keyboard.down("d"); await sleep(360);
await page.screenshot({ path: path.join(OUT, "mci_walk.png") });
await page.keyboard.up("d"); await sleep(120);
console.log("  wrote harness/shots/mci_walk.png");

// jump
await page.keyboard.down("w"); await sleep(60); await page.keyboard.up("w");
await sleep(160);
await page.screenshot({ path: path.join(OUT, "mci_jump.png") });
console.log("  wrote harness/shots/mci_jump.png");
await sleep(500);   // land

// light attack (money shot on his real art)
await page.keyboard.down("j"); await sleep(70);
await page.screenshot({ path: path.join(OUT, "mci_light.png") });
await page.keyboard.up("j");
console.log("  wrote harness/shots/mci_light.png");
await sleep(300);

// heavy (kick)
await page.keyboard.down("k"); await sleep(90);
await page.screenshot({ path: path.join(OUT, "mci_heavy.png") });
await page.keyboard.up("k");
console.log("  wrote harness/shots/mci_heavy.png");
