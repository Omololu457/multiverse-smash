// harness/miles_skins_shot.mjs — Miles Morales 11-skin batch (Default + 8 coordinated recolors +
...
// Screenshots → harness/shots/miles_skin_<id>.png.
...
const IDS = [
  ["default", ""],
  ["milesVerdantWeb", "verdantweb"],
  ["milesObsidianWeb", "obsidianweb"],
  ["milesGoldenWeb", "goldenweb"],
  ["milesFrostboundWeb", "frostboundweb"],
  ["milesVioletWeb", "violetweb"],
  ["milesEmberWeb", "emberweb"],
  ["milesAzureWeb", "azureweb"],
  ["milesAshenWeb", "ashenweb"],
  ["milesVoidSovereign", "voidsovereign"],
  ["milesUpgradedSuit", "classicsuit"],
];
const SHEETS = ["idle", "run", "jump", "fall", "air", "guard", "hurt", "knockdown", "getup", "light", "heavy", "up", "dashkick", "web", "venomstrike", "venombeam", "venomarc", "dive", "stealth", "ult", "intro", "win", "lose"];

// ── STATIC on-disk sweep — every recolored sheet + portrait exists for each of the 10 tags ──
let missing = [];
for (const [, tag] of IDS) {
  if (!tag) continue;
  for (const s of SHEETS) { const p = path.join(ROOT, `miles_${s}_uniform__${tag}.png`); if (!(fs.existsSync(p) && fs.statSync(p).size > 128)) missing.push(`${tag}:${s}`); }
  const pp = path.join(ROOT, `miles_portrait__${tag}.png`); if (!(fs.existsSync(pp) && fs.statSync(pp).size > 128)) missing.push(`${tag}:portrait`);
}
...
await pg.goto(`${base}/index.html?harness=1&p1=miles`, { waitUntil: "load" });
...
for (const [id, tag] of IDS) {
  ...
  await pg.screenshot({ path: path.join(OUT, `miles_skin_${id}.png`), clip: { x: 360, y: 300, width: 340, height: 360 } });
  await force(null); await wf(1);
  const ok = allSheets.every(s => s.includes("miles_") && (!tag || s.includes(`__${tag}`)));
...
await pg.evaluate(() => window.__harness.setSkin("p1", "milesVoidSovereign"));
await force("idle"); await wf(14);
const vf = await pg.evaluate(() => window.__harness.p1());
check("Void Sovereign applied + renders void sheet", (vf.skinId === "milesVoidSovereign") && (vf.spriteSheet || "").includes("__voidsovereign"), `skin=${vf.skinId}`);
await force(null);
