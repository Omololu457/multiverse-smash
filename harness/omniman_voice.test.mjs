// harness/omniman_voice.test.mjs — Omni-Man voice-line wiring.
...
const pool = p => page.evaluate(x => window.__harness.omnimanVoicePool(x), p);
...
try {
  ...
  for (const p of POOLS) {
    ...
    const samples = await page.evaluate(([x, n]) => window.__harness.omnimanVoicePick(x, n), [p, Math.max(200, arr.length * 25)]);
  ...
  { const log = await sfxLog(); check("an intro clip fired at match start", log.some(f => introPool.includes(f)), log.filter(f => /^omniman_/.test(f)).slice(0, 2).join(",")); }
  await page.evaluate(() => window.__harness.skipToBattle());
  await waitFrames(4);

  // ── (2) LIVE: CAST (neutral Special = Viltrumite Smash) ──
  ...
  { const log = await sfxLog(); const cast = await pool("cast"); check("special cast fires a cast-pool line", log.some(f => cast.includes(f)), log.filter(f => /^omniman_/.test(f)).join(",")); }
  ...
  { const log = await sfxLog(); const cast = await pool("cast"); check("flight activation fires a cast-pool line", log.some(f => cast.includes(f)), log.filter(f => /^omniman_/.test(f)).join(",")); }
  ...
  { const log = await sfxLog(); const cast = await pool("cast"); check("ultimate fires a cast-pool line", log.some(f => cast.includes(f)), log.filter(f => /^omniman_/.test(f)).join(",")); }
  // let the ult cinematic finish
  await page.waitForFunction(() => !window.__harness.omnimanUltCine().active, null, { timeout: 12000, polling: 16 }).catch(() => {});
  await waitFrames(3);
  ...
  { const log = await sfxLog(); const tt = await pool("taunt"); const hr = await pool("hitReact");
    check("heavy connect fires a taunt line (attacker)", log.some(f => tt.includes(f)), log.filter(f => /^omniman_/.test(f)).join(","));
  ...
  { const a = await p1(); await page.evaluate(x => window.__harness.setP2X(x), a.x + 52); }
  await page.keyboard.down("k"); await waitFrames(6); await page.keyboard.up("k"); await waitFrames(10);
  { const log = await sfxLog(); const lh = await pool("lowHealth"); check("crossing 30% fires a low-health line", log.some(f => lh.includes(f)), log.filter(f => /^omniman_/.test(f)).join(",")); }

  section("no JS errors");
  check("no page errors", jsErrors.length === 0, jsErrors.slice(0, 3).join(" | "));
} catch (e) { console.error("FATAL", e); FAIL++; }
finally {
  console.log(`\n  OMNI-MAN voice: ${PASS} passed, ${FAIL} failed`);
