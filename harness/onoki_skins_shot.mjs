// harness/onoki_skins_shot.mjs — Onoki 13-skin batch (+Default): each skin applies, renders as a SPRITE
// (never the procedural box), and resolves its recolored __<tag> sheet across multiple actions (idle/light/
// heavy/guard) so a missing recolored sheet is caught. The Eternal Void skin also seeds its amber-stone
// aura overlay. Screenshots → harness/shots/onoki_skin_<id>.png.
...
await pg.goto(`${base}/index.html?harness=1&p1=onoki`, { waitUntil: "load" });
...
const IDS = [
  ["default", ""], ["onokiStoneSovereign", "stonesovereign"], ["onokiIronFortress", "ironfortress"],
  ["onokiJadeMountain", "jademountain"], ["onokiDustRelease", "dustrelease"], ["onokiTsuchikage", "tsuchikage"],
  ["onokiGoldenKage", "goldenkage"], ["onokiThirdsRegalia", "thirdsregalia"], ["onokiMoltenCore", "moltencore"],
  ["onokiCrimsonRock", "crimsonrock"], ["onokiAshElder", "ashelder"], ["onokiYoungPrime", "youngprime"],
  ["onokiSandAccord", "sandaccord"], ["onokiEternalVoid", "eternalvoid"],
];
let boxes = 0;
for (const [id, tag] of IDS) {
  ...
  await pg.screenshot({ path: path.join(OUT, `onoki_skin_${id}.png`), clip: { x: 440, y: 180, width: 400, height: 420 } });
  const ok = allSheets.every(s => s.includes("onoki_") && (!tag || s.includes(`__${tag}`)));
  check(`${id}: renders recolored sprite (no box)`, ok, tag ? `tag=${tag}` : allSheets[0].split("/").pop());
}

// Eternal Void — the full-black skin also runs the amber-stone aura overlay: verify it seeds its FX field
// after a rendered frame (drawOnokiVoidAuraOverlay lazily seeds _onokiVoidFX on first draw).
await pg.evaluate(() => window.__harness.setSkin("p1", "onokiEternalVoid"));
await force("idle"); await wf(8);   // several rendered frames → drawOnokiVoidAuraOverlay lazily seeds + runs
const vf = await pg.evaluate(() => window.__harness.p1());
check("Eternal Void applied + renders void sheet", (vf.skinId === "onokiEternalVoid") && (vf.spriteSheet || "").includes("__eternalvoid"), `skin=${vf.skinId}`);
