// harness/raimi_shots.mjs — REAL in-game evidence that "Spider-Man (Raimi)" (spiderman_raimi) is a
...
// Usage: node harness/raimi_shots.mjs
...
// ── PART 1 — CHARACTER SELECT: Raimi appears in the marvel grid; existing chars untouched ──
...
ok(Array.isArray(roster) && roster.includes("spiderman_raimi"), "spiderman_raimi is in the character-select grid");
...
await sleep(500);
await page.screenshot({ path: path.join(OUT, "raimi_select.png") });
console.log("  wrote harness/shots/raimi_select.png");

// ── PART 2 — LIVE MATCH: Raimi spawns, renders, moves, attacks ──
...
ok(p1 && p1.key === "spiderman_raimi", `p1 is spiderman_raimi in the live match (key=${p1?.key})`);
...
await sleep(300);
await page.screenshot({ path: path.join(OUT, "raimi_idle.png") });   // idle pose
console.log("  wrote harness/shots/raimi_idle.png");

// walk right
await page.keyboard.down("d"); await sleep(360);
await page.screenshot({ path: path.join(OUT, "raimi_walk.png") });
await page.keyboard.up("d"); await sleep(120);
console.log("  wrote harness/shots/raimi_walk.png");

// jump
await page.keyboard.down("w"); await sleep(60); await page.keyboard.up("w");
await sleep(160);
await page.screenshot({ path: path.join(OUT, "raimi_jump.png") });
console.log("  wrote harness/shots/raimi_jump.png");
await sleep(500);   // land

// light attack (money shot on his real art)
await page.keyboard.down("j"); await sleep(70);
await page.screenshot({ path: path.join(OUT, "raimi_light.png") });
await page.keyboard.up("j");
console.log("  wrote harness/shots/raimi_light.png");
await sleep(300);

// heavy (kick)
await page.keyboard.down("k"); await sleep(90);
await page.screenshot({ path: path.join(OUT, "raimi_heavy.png") });
await page.keyboard.up("k");
console.log("  wrote harness/shots/raimi_heavy.png");
