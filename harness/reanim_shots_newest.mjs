// harness/reanim_shots_newest.mjs — evidence for the reanim gap RE-AUDIT (chars built since the last
// backfill): maki, miwa, ghostface, yuji, madara, green_samurai_ranger, gold_samurai_ranger. Each had ZERO
// (or, for gold, incomplete) __reanim coverage. Summons each via Tobirama's Edo Tensei, asserts the live
// vessel renders __reanim sheets (near-black reanimation palette), screenshots the reanimated body, then
// boots each normally and asserts NO __reanim anywhere (normal appearance unaffected).
//
// Also guards the two character-specific transformation OVERLAYS that must stay visually DISTINCT from the
// Edo look: Chrollo's Skill-Hunter purple tint (runtime ctx.filter, sprite.js SKILL_HUNTER_TINT) and
// Ghostface's per-identity skin modifiers. The reanim palette is a disk-only SHEET swap (retagFormAnim →
// __reanim sheets); it neither reads nor writes those overlays, so a normal (non-Edo) match of either
// character must render its canonical/identity sheet with no __reanim leak.
...
for (const vessel of ["maki", "miwa", "ghostface", "yuji", "madara", "green_samurai_ranger", "gold_samurai_ranger"]) {
