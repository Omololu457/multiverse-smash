// Clean multi-frame evidence for the Red Ranger pattern skins: renders each skin across several
// animation poses (idle / walk×2 / run / attacks) via the offscreen spriteCrop path — NO HUD, NO debug
// boxes, NO background — at TRUE render scale (gameplay size). Saves per-frame PNGs for compositing.
import { chromium } from "playwright";
import http from "node:http"; import fs from "node:fs"; import path from "node:path"; import { fileURLToPath } from "node:url";
const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const OUT = path.join(ROOT, "harness", "shots", "rr_frames"); fs.mkdirSync(OUT, { recursive: true });
...
async function crop(){ return pg.evaluate(()=>window.__harness.spriteCrop("p1")); }
function saveDataURL(u, file){ fs.writeFileSync(file, Buffer.from(u.split(",")[1], "base64")); }

const SKINS=["rr_eclipse","rr_scale"];
// (action, framesToAdvance) — covers idle, two distinct WALK frames (tracking), a RUN (fast motion), two attacks
...
for (const id of SKINS){
  ...
  for (const [action, adv] of POSES){
    await pg.evaluate(a=>window.__harness.forceAction(a,"p1"), action);
    await waitFrames(adv);
    const c = await crop();
    if (c){ saveDataURL(c.dataURL, path.join(OUT, `${id}_${n}_${action}.png`)); contentH=c.contentH; }
    n++;
  }
  await pg.evaluate(()=>window.__harness.forceAction(null,"p1"));
  console.log(`${id}: ${n} frames captured (contentH≈${contentH}px = true gameplay height)`);
}
