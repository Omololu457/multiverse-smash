// harness/samurai_red_ranger_stage5.mjs
// STAGE 5 evidence: "Fire Smasher: Blazing Strike" ULTIMATE is TIER-SCALING — same move, but base
// art + damage untransformed vs Mega art + higher damage in Mega Mode. Reuses the freeze-cinematic
// architecture (mirrors Rengoku). Fires the ult in BOTH states and confirms the art + damage differ.
// Screenshots → harness/shots/samurai_stage5_*.png.
...
async function shot(tag) { await page.screenshot({ path: path.join(OUT, `samurai_stage5_${tag}.png`) }); }
...
// Fire the ultimate and observe it through the freeze. Returns { sheet, mega, dmg }.
async function fireUltimate(tag) {
  ...
  // sample the caster's sprite sheet + tier flag mid-cinematic
  let sheet = "", mega = null;
  for (let i = 0; i < 30 && active; i++) { const c = await cine(); const a = await p1(); if (a.spriteSheet && a.spriteSheet.includes("ultimate_uniform")) sheet = a.spriteSheet; if (c.mega != null) mega = c.mega; if (c.frame > 20) { await shot(tag); break } await new Promise(r => setTimeout(r, 40)); }
  // wait for it to finish (damage applied at the strike beat)
...
try {
  ...
  check("BASE: renders the base ultimate strip", b.sheet.includes("samurai_ranger_ultimate_uniform"), `sheet=${b.sheet}`);
  ...
  // ── MEGA-TIER ULTIMATE (fresh boot → transform → ult) ────────────────
  ...
  check("MEGA: renders the MEGA ultimate strip", m.sheet.includes("samurai_ranger_mega_ultimate_uniform"), `sheet=${m.sheet}`);
  check("MEGA: cinematic tier flag = mega (mega=true)", m.mega === true, `mega=${m.mega}`);
  check("MEGA: ultimate deals damage", m.dmg > 0, `dmg=${m.dmg}`);

  // ── TIER DIFFERENCE (the stage's core assertion) ─────────────────────
