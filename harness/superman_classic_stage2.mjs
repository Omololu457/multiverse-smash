// harness/superman_classic_stage2.mjs
// STAGE 2 evidence: Superman (Custom / DCUC) 5 normals + crouchLight, from the "Attack's"/"Crouch Attack's" rows.
// (1) WIRING — every normal action's animationData → a real superman_classic_ sheet (no procedural box); up ≠ heavy.
...
// Screenshots → harness/shots/superman_classic_stage2_*.png. See SUPERMAN2_DCUC_ASSET_MAP.md.
...
async function shot(tag) { await page.screenshot({ path: path.join(OUT, `superman_classic_stage2_${tag}.png`) }); }
...
try {
  ...
  console.log("\n── (1) wiring: every normal action → a real superman_classic_ sheet (no box) ──");
  for (const [k, tag] of [
    ["light", "superman_classic_light_uniform"], ["heavy", "superman_classic_heavy_uniform"], ["up", "superman_classic_up_uniform"],
    ["air", "superman_classic_air_uniform"], ["down_air", "superman_classic_air_uniform"], ["crouchLight", "superman_classic_crouchlight_uniform"],
  ...
  for (const [name, key, tag] of [["light", "j", "superman_classic_light_uniform"], ["heavy", "k", "superman_classic_heavy_uniform"], ["up", "i", "superman_classic_up_uniform"]]) {
    await prep(50);
  ...
  await page.keyboard.down("j"); let sawAir = false; for (let i = 0; i < 5; i++) { const a = await p1(); if ((a.spriteSheet || "").includes("superman_classic_air_uniform")) sawAir = true; await waitFrames(2); } await page.keyboard.up("j");
  await shot("air"); check("air (airborne J) renders superman_classic_air_uniform", sawAir, "");
  await waitGrounded();
  await prep(28); await page.evaluate(() => window.__harness.jumpP1?.()); await waitFrames(4);
  await page.keyboard.down("s"); await page.keyboard.down("j"); let sawDA = false; for (let i = 0; i < 5; i++) { const a = await p1(); if ((a.spriteSheet || "").includes("superman_classic_air_uniform")) sawDA = true; await waitFrames(2); } await page.keyboard.up("j"); await page.keyboard.up("s");
  await shot("down_air"); check("down_air (airborne S+J) renders superman_classic_air_uniform (reuse)", sawDA, "");
  await waitGrounded();
  ...
  await page.keyboard.down("j"); let sawCr = false; for (let i = 0; i < 5; i++) { const a = await p1(); if ((a.spriteSheet || "").includes("superman_classic_crouchlight_uniform")) sawCr = true; await waitFrames(2); } await page.keyboard.up("j");
  await page.keyboard.up("s"); await waitFrames(6);
  const crDmg = hc0 - (await p2()).health;
  await shot("crouchlight");
  check("crouchLight (S+J) renders superman_classic_crouchlight_uniform", sawCr, "");
