// harness/superman_classic_stage4.mjs
...
// All projectile/melee damage runs through GLOBAL_DAMAGE_SCALE ×0.60. Screenshots → harness/shots/superman_classic_stage4_*.png.
...
async function shot(tag) { await page.screenshot({ path: path.join(OUT, `superman_classic_stage4_${tag}.png`) }); }
...
try {
  ...
  await waitFrames(6);
  const ad = await page.evaluate(() => window.__harness.charDef("superman_classic").animationData);

  console.log("\n── (1) wiring: special melee actions → real superman_classic_ sheets (no box) ──");
  for (const [k, tag] of [
    ["supClsHeat", "superman_classic_heatvision_uniform"], ["supClsFlycharge", "superman_classic_flycharge_uniform"], ["supClsSoar", "superman_classic_up_uniform"], ["supClsDive", "superman_classic_dive_uniform"],
  ]) check(`${k} wired → ${tag}`, (ad[k]?.sheet || "").includes(tag), `sheet=${ad[k]?.sheet}`);

  console.log("\n── (2) Heat Vision (neutral) — REAL baked-beam long DISJOINT melee: connects from far + renders sprite ──");
  await prep(130);   // far gap — proves the long beam reach (rangeX 150)
  let h0 = (await p2()).health;
  const hres = await fireDir(null);
  check(`Heat Vision fires supClsHeat (melee disjoint, not a projectile)`, hres?.move === "supClsHeat", `move=${hres?.move}`);
  let sawHeat = false; for (let f = 0; f < 12; f++) { const mv = await p1(); if ((mv.spriteSheet || "").includes("superman_classic_heatvision_uniform")) sawHeat = true; await waitFrames(1); }
  check(`Heat Vision renders superman_classic_heatvision_uniform (baked beam)`, sawHeat, "");
  await shot("heatvision"); await waitFrames(6);
  check(`Heat Vision connects from far (dmg ${((h0 - (await p2()).health)).toFixed(0)})`, h0 - (await p2()).health > 0, "");
  ...
  check(`Flying Charge fires supClsFlycharge`, cres?.move === "supClsFlycharge", `move=${cres?.move}`);
  ...
  for (let f = 0; f < 14; f++) { const mv = await p1(); if ((mv.spriteSheet || "").includes("superman_classic_flycharge_uniform")) sawCharge = true; maxDx = Math.max(maxDx, Math.abs(mv.x - cx0)); chDealt = Math.max(chDealt, h0 - (await p2()).health); await waitFrames(1); }
  check(`Flying Charge renders superman_classic_flycharge_uniform`, sawCharge, "");
  ...
  check(`Soaring Uppercut fires supClsSoar`, ures?.move === "supClsSoar", `move=${ures?.move}`);
  let sawSoar = false, launched = { grounded: true, vy: 0 }, soDealt = 0;
  for (let f = 0; f < 14; f++) { const mv = await p1(); if ((mv.spriteSheet || "").includes("superman_classic_up_uniform")) sawSoar = true; const b = await p2(); if (!b.grounded || b.vy < -0.5) launched = b; soDealt = Math.max(soDealt, h0 - b.health); await waitFrames(1); }
  ...
  // ── (5) ICE BREATH (Down) — REAL frost-cloud projectile (freeze/slow). Spawn/width at a far gap, then connect.
  await prep(200);   // far enough that the short-lived frost gust stays visible before contact
  await fireDir("D");
  const frost = await seeProj("supClsIce", 20);
  check(`Ice Breath spawns a real frost-cloud projectile (w=${frost.maxW})`, frost.seen >= 1 && frost.maxW >= 40, `seen=${frost.seen} w=${frost.maxW}`);
  const frostSheet = await page.evaluate(() => (window.__harness.projectiles().find(p => (p.name||"").toLowerCase().includes("supclsice"))||{}).sheet || "");
  await shot("icebreath");
  await waitGrounded(); await waitFrames(6);
  // Connect proof at close range (freeze = big hitstun).
  await prep(60);
  h0 = (await p2()).health;
  await fireDir("D");
  await waitFrames(16);
  check(`Ice Breath connects (dmg ${((h0 - (await p2()).health)).toFixed(0)})`, h0 - (await p2()).health > 0, "");
  await waitGrounded(); await waitFrames(6);
  ...
  check(`air Special fires supClsDive`, fres?.move === "supClsDive", `move=${fres?.move}`);
  ...
  for (let f = 0; f < 16; f++) { const mv = await p1(); if ((mv.spriteSheet || "").includes("superman_classic_dive_uniform")) sawDive = true; dvDx = Math.max(dvDx, Math.abs(mv.x - fx0)); dvDealt = Math.max(dvDealt, h0 - (await p2()).health); await waitFrames(1); }
  check(`Flying Dive Kick renders superman_classic_dive_uniform`, sawDive, "");
  await shot("dive");
