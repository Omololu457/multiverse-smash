// harness/superman_dcuc_stage4.mjs
...
// All projectile/melee damage runs through GLOBAL_DAMAGE_SCALE ×0.60. Screenshots → harness/shots/superman_dcuc_stage4_*.png.
...
async function shot(tag) { await page.screenshot({ path: path.join(OUT, `superman_dcuc_stage4_${tag}.png`) }); }
...
try {
  ...
  console.log("\n── (1) wiring: special melee actions → real superman_dcuc_ sheets (no box) ──");
  for (const [k, tag] of [
    ["supDcucFlycharge", "superman_dcuc_flycharge_uniform"], ["supDcucSoar", "superman_dcuc_up_uniform"], ["supDcucDive", "superman_dcuc_dive_uniform"],
  ...
  await fireDir(null);
  const beam = await seeProj("supHeatVision", 20);
  ...
  check(`Flying Charge fires supDcucFlycharge`, cres?.move === "supDcucFlycharge", `move=${cres?.move}`);
  ...
  for (let f = 0; f < 14; f++) { const mv = await p1(); if ((mv.spriteSheet || "").includes("superman_dcuc_flycharge_uniform")) sawCharge = true; maxDx = Math.max(maxDx, Math.abs(mv.x - cx0)); chDealt = Math.max(chDealt, h0 - (await p2()).health); await waitFrames(1); }
  check(`Flying Charge renders superman_dcuc_flycharge_uniform`, sawCharge, "");
  ...
  check(`Soaring Uppercut fires supDcucSoar`, ures?.move === "supDcucSoar", `move=${ures?.move}`);
  let sawSoar = false, launched = { grounded: true, vy: 0 }, soDealt = 0;
  for (let f = 0; f < 14; f++) { const mv = await p1(); if ((mv.spriteSheet || "").includes("superman_dcuc_up_uniform")) sawSoar = true; const b = await p2(); if (!b.grounded || b.vy < -0.5) launched = b; soDealt = Math.max(soDealt, h0 - b.health); await waitFrames(1); }
  ...
  await prep(260);
  await fireDir("D");
  const gust = await seeProj("supBreath", 22);
  ...
  check(`air Special fires supDcucDive`, fres?.move === "supDcucDive", `move=${fres?.move}`);
  ...
  for (let f = 0; f < 16; f++) { const mv = await p1(); if ((mv.spriteSheet || "").includes("superman_dcuc_dive_uniform")) sawDive = true; dvDx = Math.max(dvDx, Math.abs(mv.x - fx0)); dvDealt = Math.max(dvDealt, h0 - (await p2()).health); await waitFrames(1); }
  check(`Flying Dive Kick renders superman_dcuc_dive_uniform`, sawDive, "");
  await shot("dive");
