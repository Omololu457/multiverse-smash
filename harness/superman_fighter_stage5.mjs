// harness/superman_fighter_stage5.mjs
// STAGE 5 evidence: Superman (New 52) ULTIMATE "Kryptonian Detonation".
//   • p1Ultimate → cast=true, spends 100 Solar Energy, LIVE fighter holds the explosion pose (supFtrUlt, no dup)
...
// Screenshots → harness/shots/superman_fighter_stage5_*.png.
...
async function shot(tag) { await page.screenshot({ path: path.join(OUT, `superman_fighter_stage5_${tag}.png`) }); }
...
try {
  ...
  check("Kryptonian Detonation fires on the LIVE fighter (cast=true)", r?.cast === true, `cast=${r?.cast} castMove=${r?.castMove}`);
  check("caster holds the explosion cast pose (castMove = supFtrUlt)", r?.castMove === "supFtrUlt", `castMove=${r?.castMove}`);
  const en1 = (await p1()).energy;
  check("ultimate spent the 100 Solar Energy cost", (en0 - en1) >= 95, `energy ${en0} → ${en1} (−${(en0 - en1).toFixed(0)})`);

  console.log("\n── live fighter holds the REAL explosion cast + 3-beat guaranteed barrage ──");
  let castSheet = false, box = false, landed = false, hpAfter = hp0;
  for (let i = 0; i < 54; i++) {
    const g = await p1(); const sh = g.spriteSheet || "";
    if (sh.includes("superman_fighter_ult_uniform")) castSheet = true;
    if (!sh.includes("superman_fighter_")) box = true;
    hpAfter = (await p2()).health; if (hpAfter < hp0) landed = true;
    if (i === 10) await shot("beam"); if (i === 24) await shot("payoff");
    await waitFrames(1);
  }
  check("live fighter renders the Kryptonian Detonation beam cast (no box)", castSheet && !box, `castSheet=${castSheet} box=${box}`);
  ...
  const gEnd = await p1();
  check("fighter recovers to a normal action after the ult", (gEnd.spriteSheet || "").includes("superman_fighter_") && gEnd.hasSpriteHandler, `action=${gEnd.spriteAction} sheet=${gEnd.spriteSheet}`);
