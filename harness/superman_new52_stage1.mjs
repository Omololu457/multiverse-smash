// harness/superman_new52_stage1.mjs
// STAGE 1 evidence: Superman (Custom / DC Universe Customs), rosterKey "superman_new52", DC.
...
// superman_new52_*_uniform.png sheets (HONEST reuses: dash=run, guard/getup/charge=idle, descentLand=crouch),
// measures on-screen body height, sweeps for the procedural fallback box, and confirms the built `superman`
// (Arcade) stays a SEPARATE entry with its OWN art. Screenshots → harness/shots/superman_new52_stage1_*.png.
...
async function boot() { await page.goto(`${base}/index.html?harness=1&p1=superman_new52`, { waitUntil: "load" }); await page.waitForFunction(() => !!window.__harness, null, { timeout: 15000 }); await page.mouse.click(640, 360); }
async function waitFrames(n) { const s = (await state()).frame; await page.waitForFunction(([a, b]) => window.__harness.state().frame >= a + b, [s, n], { timeout: 15000, polling: 16 }); }
async function waitGrounded() { await page.waitForFunction(() => { const p = window.__harness.p1(); return p.grounded && Math.abs(p.vy) < 0.5; }, null, { timeout: 8000, polling: 16 }).catch(() => {}); }
async function shot(tag) { await page.screenshot({ path: path.join(OUT, `superman_new52_stage1_${tag}.png`) }); }
...
try {
  ...
  check("idle sheet = superman_new52_idle_uniform", (g.spriteSheet || "").includes("superman_new52_idle_uniform"), `sheet=${g.spriteSheet}`);
  check("spriteScale = 1.25", Math.abs((g.spriteScale || 0) - 1.25) < 0.01, `spriteScale=${g.spriteScale}`);
  check("maxHealth = 1250", g.maxHealth === 1250, `HP=${g.maxHealth}`);
  check("maxEnergy = 200", g.maxEnergy === 200, `EN=${g.maxEnergy}`);
  const def = await page.evaluate(() => window.__harness.charDef("superman_new52"));
  ...
  check("portrait asset exists on disk", fs.existsSync(path.join(ROOT, "superman_new52_portrait.png")), "");
  ...
  check("walk → superman_new52_idle_uniform (borrow — genuine walk gap)", /superman_new52_idle_uniform/.test(wk.spriteSheet || ""), `action=${wk.spriteAction} sheet=${wk.spriteSheet}`);

  await force("run"); await waitFrames(4); const rn = await p1(); await shot("run"); await force(null); await waitFrames(2);
  check("run → superman_new52_idle_uniform (borrow — walk gap)", (rn.spriteSheet || "").includes("superman_new52_idle_uniform"), `sheet=${rn.spriteSheet}`);

  await force("dash"); await waitFrames(3); const ds = await p1(); await shot("dash"); await force(null); await waitFrames(2);
  check("dash → superman_new52_idle_uniform (borrow — walk gap)", (ds.spriteSheet || "").includes("superman_new52_idle_uniform"), `sheet=${ds.spriteSheet}`);

  await force("crouch"); await waitFrames(4); const cr = await p1(); await shot("crouch"); await force(null); await waitFrames(2);
  check("crouch → superman_new52_crouch_uniform", (cr.spriteSheet || "").includes("superman_new52_crouch_uniform"), `sheet=${cr.spriteSheet}`);
  ...
  check("jump uses superman_new52_jump_uniform", (jp.spriteSheet || "").includes("superman_new52_jump_uniform"), `action=${jp.spriteAction} sheet=${jp.spriteSheet}`);
  await waitGrounded();

  await force("fall"); await waitFrames(3); const fl = await p1(); await shot("fall"); await force(null); await waitFrames(2);
  check("fall → superman_new52_jump_uniform (reuse rise — no fall art)", (fl.spriteSheet || "").includes("superman_new52_jump_uniform"), `sheet=${fl.spriteSheet}`);

  await force("guard"); await waitFrames(4); const bl = await p1(); await shot("guard"); await force(null); await waitFrames(2);
  check("guard → superman_new52_idle_uniform (reuse)", (bl.spriteSheet || "").includes("superman_new52_idle_uniform"), `action=${bl.spriteAction} sheet=${bl.spriteSheet}`);

  await force("hurt"); await waitFrames(4); const ht = await p1(); await shot("hurt"); await force(null); await waitFrames(2);
  check("hurt → superman_new52_hurt_uniform (real recoil, distinct from idle)", (ht.spriteSheet || "").includes("superman_new52_hurt_uniform"), `sheet=${ht.spriteSheet}`);

  await force("knockdown"); await waitFrames(3); const kd = await p1(); await shot("knockdown"); await force(null); await waitFrames(2);
  check("knockdown → superman_new52_knockdown_uniform (real knockback art)", (kd.spriteSheet || "").includes("superman_new52_knockdown_uniform"), `sheet=${kd.spriteSheet}`);

  console.log("\n── flight (canFly) ──");
  await force("fly"); await waitFrames(4); const fy = await p1(); await shot("fly"); await force(null); await waitFrames(2);
  check("fly → superman_new52_fly_uniform", (fy.spriteSheet || "").includes("superman_new52_fly_uniform"), `action=${fy.spriteAction} sheet=${fy.spriteSheet}`);
  ...
  for (const act of ["idle", "walk", "run", "dash", "crouch", "jump", "fall", "guard", "hurt", "knockdown", "fly"]) {
    ...
    if (!sh.includes("superman_new52_")) boxHit.push(`${act}:${sh || "null"}`);
    await force(null); await waitFrames(1);
  }
  check("every movement action resolves a real superman_new52_ sheet (no box)", boxHit.length === 0, boxHit.join(" | "));
  ...
  check("superman_new52 uses its OWN art (distinct from superman)", dcSheet.includes("superman_new52") && oldSheet.includes("superman_idle") && !oldSheet.includes("new52"), `new52=${dcSheet} old=${oldSheet}`);
  check("distinct stats (new52 1250 vs superman 1450)", def?.stats?.maxHealth === 1250 && oldS?.stats?.maxHealth === 1450, `new52=${def?.stats?.maxHealth} old=${oldS?.stats?.maxHealth}`);
