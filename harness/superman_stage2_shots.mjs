// harness/superman_stage2_shots.mjs — STAGE 2: 5 normals + "Kryptonian Rush" command chain
// (Fwd+Heavy 3-hit rekka supRush1→supRush2→supRushFin launcher) with a mid-chain interrupt test.
...
async function shot(name) { await page.screenshot({ path: path.join(OUT, `superman_s2_${name}.png`) }); }
const seen = new Set();
...
await shot("rush1");
for (const want of ["supRush2", "supRushFin"]) {
  ...
  await shot(want === "supRush2" ? "rush2" : "rushfin");
}
await waitFrames(24);
const chainDmg = hp0 - (await p2()).health;
check("chain sequences supRush1→supRush2→supRushFin", seq.join("→") === "supRush1→supRush2→supRushFin", `seq=${seq.join("→")}`);
...
check("interrupt: opener fired (supRush1)", openMove === "supRush1", `open=${openMove}`);
check("interrupt: whiff does NOT advance to supRush2", afterMove !== "supRush2" && afterMove !== "supRushFin", `after=${afterMove || "(ended)"}`);
...
console.log(`\nRESULT ${PASS} pass / ${FAIL} fail — shots in harness/shots/superman_s2_*.png`);
