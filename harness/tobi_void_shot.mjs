...
const OUT = path.join(ROOT, "harness", "shots", "tobi_void"); fs.mkdirSync(OUT, { recursive: true });
...
async function cropShot(tag){
  ...
  const cx=r.x+r.w/2, cy=r.y+r.h/2, cw=Math.min(520,r.w*3.4), chh=Math.min(440,r.h*2.0);
...
await pg.goto(`${base}/index.html?harness=1&p1=tobi`,{waitUntil:"load"});
await pg.waitForFunction(()=>!!window.__harness,null,{timeout:15000}); await pg.mouse.click(640,360);
await pg.evaluate(()=>{window.__harness.start({mode:"vs",difficulty:"easy",p1Skin:"tobiKamuiVoid"}); window.__harness.skipToBattle();});
await waitFrames(6); await pushP2();
// IDLE — capture several frames to catch a Kamui swirl-pulse mid-bloom
for (let i=0;i<7;i++){ await pushP2(); await waitFrames(9); await cropShot(`idle_${i}`); }
// ATTACK — force a heavy pose, overlay tracks
