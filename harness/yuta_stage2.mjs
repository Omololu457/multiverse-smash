// harness/yuta_stage2.mjs — STAGE 2: Yuta's 5 basic normals.
// For light(J)/heavy(K)/upAttack(I)/air(J airborne)/downAir(S+J airborne):
//   (1) it resolves the correct yuta_*_uniform sheet (no 128² fallback box)
//   (2) it CONNECTS on the adjacent dummy (p2 health drops)
// Plus a data-level contract that every normal is wired to a real yuta sheet. down_air REUSES the air
// sheet (honest reuse — no dedicated down-aerial art). Yuta has NO crouch-attack variant (crouch = idle).
...
async function crop(name) {
  ...
  if (!r) { await page.screenshot({ path: path.join(OUT, `yuta_s2_${name}.png`) }); return; }
  ...
  await page.screenshot({ path: path.join(OUT, `yuta_s2_${name}_crop.png`), clip });
}
...
try {
  ...
  console.log("\n── ground normals ──");
  const ground = [
    ["light", "j", "yuta_light_uniform"],
    ["heavy", "k", "yuta_heavy_uniform"],
    ["upAttack", "i", "yuta_up_uniform"],
  ];
  ...
  await setupAdjacent(44);
  {
    ...
    const mv = await waitSheet("yuta_air_uniform");
    check(`air: sprite → yuta_air_uniform`, (mv.spriteSheet || "").includes("yuta_air_uniform"), `action=${mv.action} sheet=${mv.spriteSheet}`);
    await crop("air");
  ...
  await setupAdjacent(30);
  {
    ...
    const mv = await waitSheet("yuta_air_uniform");
    check(`downAir: sprite → yuta_air_uniform (reuse)`, (mv.spriteSheet || "").includes("yuta_air_uniform"), `action=${mv.action} sheet=${mv.spriteSheet}`);
    await crop("downAir");
  ...
  // ── DATA-LEVEL contract: all 5 normals wired to real yuta sheets ──
  console.log("\n── data contract ──");
  ...
  check("all 5 normals wired to real yuta sheets", allWired, JSON.stringify(Object.fromEntries(keys.map(k => [k, (ad[k]?.sheet || "MISSING").split("/").pop()]))));
  ...
  console.log(`\n${FAIL === 0 ? "✅" : "❌"} Yuta Stage 2: ${PASS} passed, ${FAIL} failed — shots in harness/shots/yuta_s2_*_crop.png`);
