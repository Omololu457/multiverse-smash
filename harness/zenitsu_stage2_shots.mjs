// harness/zenitsu_stage2_shots.mjs — STAGE 2: 5 normals + "Thunderclap Flurry" command chain
// (Down+Heavy 3-hit rekka zenCombo1→2→3) with a mid-chain interrupt test, plus a no-projectile
// sweep. Captures screenshots of each connect. Mirrors flash_stage2_shots.mjs.
...
async function shot(name) { await page.screenshot({ path: path.join(OUT, `zenitsu_s2_${name}.png`) }); }
const seen = new Set();
...
console.log("\n── 5 normals ──");
for (const [name, key, gap, sheet] of [["light", "j", 58, "zenitsu_foward_slash_uniform"], ["heavy", "k", 64, "zenitsu_foward_hit_uniform"], ["up", "i", 56, "zenitsu_up_attack_uniform"]]) {
  await prep(gap);
  ...
  check(`${name} connects & sheet`, d > 0 && (a.spriteSheet || seen.has(`./${sheet}.png`)), `−${d.toFixed(0)} sheet=${a.spriteSheet}`);
}
// air
await waitGrounded(); await prep(52);
{ const d = await dmgFrom(async () => { await page.evaluate(() => window.__harness.liftP1(44)); await page.keyboard.down("j"); await waitFrames(3); const a = await record(); if (a.spriteSheet) seen.add(a.spriteSheet); await shot("air"); await page.keyboard.up("j"); }); check("air connects", d > 0, `−${d.toFixed(0)}`); }
// down_air
await waitGrounded(); await prep(30);
{ const d = await dmgFrom(async () => { await page.evaluate(() => window.__harness.liftP1(50)); await page.keyboard.down("s"); await page.keyboard.down("j"); await waitFrames(3); const a = await record(); if (a.spriteSheet) seen.add(a.spriteSheet); await shot("down_air"); await page.keyboard.up("j"); await page.keyboard.up("s"); }); check("down_air connects", d > 0, `−${d.toFixed(0)}`); }

console.log("\n── Thunderclap Flurry command chain (Down+Heavy 3-hit rekka) ──");
await waitGrounded(); await prep(44);
...
await shot("combo1");
for (const want of ["zenCombo2", "zenCombo3"]) {
  ...
  await shot(want === "zenCombo2" ? "combo2" : "combo3");
}
await waitFrames(20);
const chainDmg = hp0 - (await p2()).health;
check("chain sequences zenCombo1→2→3", seq.join("→") === "zenCombo1→zenCombo2→zenCombo3", `seq=${seq.join("→")}`);
check("chain deals combo damage", chainDmg > 0, `−${chainDmg.toFixed(0)}`);

console.log("\n── mid-chain interrupt (whiff → chain stops) ──");
await waitGrounded(); await prep(44);
await page.evaluate(x => window.__harness.setP2X(x + 600), 0);   // shove P2 far away → opener WHIFFS
await page.keyboard.down("s"); await page.keyboard.down("k"); await waitFrames(2); await page.keyboard.up("k"); await page.keyboard.up("s");
const openMove = (await p1()).currentMove;
let rec = false; for (let i = 0; i < 40; i++) { const p = await record(); if (!p.attacking) break; if (p.attackPhase === "recovery") { rec = true; break; } await waitFrames(1); }
if (rec) await tapKey("k");
const afterMove = (await p1()).currentMove;
check("interrupt: opener fired", openMove === "zenCombo1", `open=${openMove}`);
check("interrupt: whiff does NOT advance to zenCombo2", afterMove !== "zenCombo2", `after=${afterMove || "(ended)"}`);

console.log("\n── no-projectile sweep (pure melee) ──");
...
console.log(`\nRESULT ${PASS} pass / ${FAIL} fail — shots in harness/shots/zenitsu_s2_*.png`);
