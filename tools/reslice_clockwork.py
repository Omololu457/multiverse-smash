#!/usr/bin/env python3
# Re-slice CLOCKWORK (ipmugen JPEG "clocwork...", cyan bg) -> uniform strips (CC-box scheme, JPEG cyan key).
import sys, numpy as np
from PIL import Image, ImageDraw, ImageFont
from scipy import ndimage
SCALE=0.68
SRC="clocwork_sprites___ben_10_original_game_by_ipmugenofficial_djpbjqa-fullview.jpeg"; BAND=66; FLIP_H=False
def load():
    a=np.asarray(Image.open(SRC).convert("RGB")).astype(int)
    bg=(a[:,:,0]<95)&(a[:,:,1]>135)&(a[:,:,2]>175)
...
PICKS={"idle":[0,1,2,3],"walk":[15,16,17,18],"jump":[8],"crouch":[10],
       "light":[27,28],"heavy":[29,30],"up":[36],"air":[43,44],"ray":[24,25,26]}
def main():
    ...
    for act,picks in PICKS.items():
        ...
        if picks: reslice(im,boxes,f"ben10_clockwork_{act}_uniform.png",picks)
def verify():
    ...
    for i,act in enumerate(acts):
        im=Image.open(f"ben10_clockwork_{act}_uniform.png").convert("RGBA")
    ...
    mont.convert("RGB").save("harness/shots/s3_clockwork_verify.png"); print("wrote s3_clockwork_verify.png")
