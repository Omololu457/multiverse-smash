#!/usr/bin/env python3
# Re-slice EYE GUY (Dragonrod sheet, teal-gray bg #A7C9C8) -> feet-aligned uniform strips (CC-box scheme).
import sys, numpy as np
from PIL import Image, ImageDraw, ImageFont
from scipy import ndimage
SRC="eyeguy_sprite_sheet_by_dragonrod342_d82qnsd.png"; BAND=44; FLIP_H=False
def load():
    a=np.asarray(Image.open(SRC).convert("RGB")).astype(int)
    bg=(np.abs(a-np.array([167,201,200])).sum(2)<50)|(a.sum(2)>715)
...
PICKS={"idle":[0,1,2,3],"walk":[8,9,10,11,12,13],"jump":[4],"crouch":[7],
       "light":[17,18],"heavy":[20,21],"up":[28,29],"air":[26,27],"beam":[20,21,22]}
def main():
    ...
    for act,picks in PICKS.items():
        ...
        if picks: reslice(im,boxes,f"ben10_eyeguy_{act}_uniform.png",picks)
def verify():
    acts=list(PICKS.keys()); cw=220; ch=100
    ...
    for i,act in enumerate(acts):
        im=Image.open(f"ben10_eyeguy_{act}_uniform.png").convert("RGBA")
    ...
    mont.convert("RGB").save("harness/shots/s3_eyeguy_verify.png"); print("wrote s3_eyeguy_verify.png")
