#!/usr/bin/env python3
# Transcribe the 14 tobi_voice_*.mp3 clips (Tobi (masked Obito alias), Japanese fighting-game voice rip).
# Same faster-whisper + VAD pipeline proven this session (Miwa/Vegeta/Sukuna/etc.), but TWO passes per
# clip because Madara's kit is technique-name-heavy and the callouts (Katon/Susanoo/Mokuton/Tengai) are
...
# tobi_raw_transcript.tsv (resumable). Cols: idx, file, dur, lang, langprob, ja_text, en_gloss.
import glob, os, sys
from faster_whisper import WhisperModel

files = sorted(glob.glob("tobi_voice_*.mp3"))
OUT = "tobi_raw_transcript.tsv"
done = set()
