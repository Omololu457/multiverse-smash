// ui.js
// Shared UI rendering and menu layout helpers for:
// - Start screen
// - Gameplay mode select
...
function drawHeader(ctx, canvas, title, subtitle = "") {
  ...
  if (subtitle) {
    drawSubText(ctx, subtitle, w / 2, 112, {
      font: "18px Arial",
      fill: "rgba(220,230,255,0.78)"
    })
  }
}
...
// ------------------------------
// MENU LAYOUT EXPORTS
// These are for click detection in game.js later.
// ------------------------------
  ...
  return getVerticalMenuLayout(canvas, [
    {
      id: "training",
      label: "TRAINING",
      subLabel: "1 player practice mode"
    },
    {
      id: "vs",
      label: "VS MATCH",
      subLabel: "Fight the CPU"
    },
    {
      id: "back",
      label: "BACK",
  ...
  return getVerticalMenuLayout(canvas, [
    {
      id: "easy",
      label: "EASY",
      subLabel: "Low pressure, simple AI"
    },
    {
      id: "adaptive",
      label: "ADAPTIVE",
      subLabel: "Learns and responds"
    },
    {
      id: "impossible",
      label: "IMPOSSIBLE",
      subLabel: "Aggressive and relentless"
    },
    {
      id: "back",
      label: "BACK",
      subLabel: "Return to mode select"
    }
  ])
}
...
export function getUniverseCardRects(canvas, universes = []) {
  ...
  return getGridLayout(universes.length, canvas, {
    cols: 3,
    cardW: 300,
    cardH: 110,
    gap: 24,
...
export function getCharacterCardRects(canvas, roster = []) {
  ...
  return getGridLayout(roster.length, canvas, {
    cols: 4,
    cardW: 220,
    cardH: 110,
    gap: 18,
...
export function getStageCardRects(canvas, stages = []) {
  ...
  return getGridLayout(stages.length, canvas, {
    cols: 3,
    cardW: 310,
    cardH: 120,
    gap: 24,
...
export function drawGameplaySelectScreen(ctx, canvas, selectedIndex = 0) {
  ...
  const buttons = getGameplaySelectRects(canvas)
...
export function drawAIDifficultyScreen(ctx, canvas, selectedIndex = 0) {
  ...
  const buttons = getAIDifficultyRects(canvas)
  buttons.forEach((button, index) => {
    drawButton(ctx, button, {
      label: button.label,
      subLabel: button.subLabel,
...
export function drawUniverseSelectScreen(ctx, canvas, universes = [], selectedIndex = 0) {
  ...
  if (!universes.length) {
    drawSubText(ctx, "No universes available", w / 2, h / 2, {
      font: "20px Arial"
    })
    return
  }
  ...
  universes.forEach((universe, i) => {
    ...
    drawCenteredText(
      ctx,
      universe?.name || universe?.id || universe?.label || `Universe ${i + 1}`,
      rect.x + rect.w / 2,
      rect.y + 58,
      {
        font: "700 24px Arial",
...
export function drawCharacterSelectScreen(ctx, canvas, options = {}) {
  ...
  if (!roster.length) {
    drawSubText(ctx, "No fighters available", w / 2, h / 2, {
      font: "20px Arial"
    })
    return
  }
  ...
  roster.forEach((fighter, i) => {
    ...
    if (isCursor) {
      fill = "rgba(70,110,210,0.35)"
      stroke = "#dbe7ff"
    }

    if (isP1 && isP2) {
      fill = "rgba(180,120,255,0.35)"
      stroke = "#d7b8ff"
    } else if (isP1) {
      fill = "rgba(70,190,255,0.28)"
      stroke = "#7fd3ff"
    } else if (isP2) {
    ...
    if (universe) {
      drawSubText(ctx, universe, rect.x + rect.w / 2, rect.y + 68, {
        font: "13px Arial",
        fill: "rgba(220,230,255,0.72)"
      })
    }

    if (isP1) {
      drawCenteredText(ctx, "P1", rect.x + 18, rect.y + 16, {
        font: "700 12px Arial",
        fill: "#7fd3ff",
        align: "left",
        baseline: "alphabetic"
    ...
    if (isP2) {
      drawCenteredText(ctx, "P2", rect.x + rect.w - 18, rect.y + 16, {
        font: "700 12px Arial",
        fill: "#ff9f9f",
        align: "right",
        baseline: "alphabetic"
...
export function drawStageSelectScreen(ctx, canvas, stages = [], selectedIndex = 0) {
  ...
  if (!stages.length) {
    drawSubText(ctx, "No stages available", w / 2, h / 2, {
      font: "20px Arial"
    })
    return
  }
  ...
  stages.forEach((stage, i) => {
    ...
    drawPanel(ctx, rect.x, rect.y, rect.w, rect.h, {
      fill: "rgba(0,0,0,0)",
      stroke: isSelected ? "#e8efff" : "rgba(255,255,255,0.18)",
      lineWidth: isSelected ? 3 : 2
    })

    drawCenteredText(
      ctx,
      stage?.name || stage?.id || `Stage ${i + 1}`,
      rect.x + rect.w / 2,
      rect.y + rect.h - 28,
      {
        font: "700 18px Arial",
...
export function drawBattleBackground(ctx, canvas, stage = {}, groundY = 600, floorHeight = 120) {
  ...
  const worldWidth = Number.isFinite(parsedWorldWidth)
    ? Math.max(parsedWorldWidth, w)
    : w
  ...
  if (stage?.name) {
    ...
    drawCenteredText(ctx, stage.name, 147, 44, {
      font: "700 16px Arial",
      fill: accent
    })
  }
...
export function drawFighter(ctx, fighter, camera = null) {
  ...
  const faceRight =
    typeof fighter.facingRight === "boolean" ? fighter.facingRight : (fighter.facing ?? 1) >= 0

  const bodyColor =
    fighter.color ||
    fighter.fill ||
    (fighter.isCPU ? "#ff8b8b" : "#8ec5ff")
  ...
  if (faceRight) {
    ctx.fillRect(x + width - 18, y + 18, 8, 8)
  } else {
...
export function drawHealthAndEnergyBars(ctx, p1, p2, canvas) {
  const { width: w } = getCanvasSize(canvas)
  const barW = clamp(w * 0.26, 240, 420)
  const barH = 22
  const topY = 28

  const p1HealthMax = Math.max(1, p1?.maxHealth || 100)
  const p2HealthMax = Math.max(1, p2?.maxHealth || 100)
  const p1EnergyMax = Math.max(1, p1?.maxEnergy || 100)
  const p2EnergyMax = Math.max(1, p2?.maxEnergy || 100)

  const p1HealthRatio = clamp((p1?.health ?? p1HealthMax) / p1HealthMax, 0, 1)
  const p2HealthRatio = clamp((p2?.health ?? p2HealthMax) / p2HealthMax, 0, 1)
  const p1EnergyRatio = clamp((p1?.energy ?? p1EnergyMax) / p1EnergyMax, 0, 1)
  const p2EnergyRatio = clamp((p2?.energy ?? p2EnergyMax) / p2EnergyMax, 0, 1)

  // Panels
  drawPanel(ctx, 26, 18, barW + 24, 76, {
    radius: 16,
  ...
  drawPanel(ctx, w - barW - 50, 18, barW + 24, 76, {
    radius: 16,
    fill: "rgba(0,0,0,0.42)",
    stroke: "rgba(255,255,255,0.14)",
    lineWidth: 1
  ...
  // P1
  drawCenteredText(ctx, p1?.name || "P1", 50, 34, {
    font: "700 14px Arial",
    fill: "#8fd0ff",
    align: "left",
    baseline: "alphabetic"
  ...
  ctx.fillRect(38, topY + 14, barW, barH)
  ctx.fillStyle = "#52d46b"
  ctx.fillRect(38, topY + 14, barW * p1HealthRatio, barH)

  ctx.fillStyle = "rgba(255,255,255,0.12)"
  ctx.fillRect(38, topY + 44, barW, 12)
  ctx.fillStyle = "#5bb4ff"
  ctx.fillRect(38, topY + 44, barW * p1EnergyRatio, 12)

  // P2
  drawCenteredText(ctx, p2?.name || "P2", w - 50, 34, {
    font: "700 14px Arial",
    fill: "#ff9f9f",
    align: "right",
    baseline: "alphabetic"
  ...
  ctx.fillStyle = "rgba(255,255,255,0.12)"
  ctx.fillRect(w - 38 - barW, topY + 14, barW, barH)
  ctx.fillStyle = "#52d46b"
  ctx.fillRect(w - 38 - barW + barW * (1 - p2HealthRatio), topY + 14, barW * p2HealthRatio, barH)

  ctx.fillStyle = "rgba(255,255,255,0.12)"
  ctx.fillRect(w - 38 - barW, topY + 44, barW, 12)
  ctx.fillStyle = "#5bb4ff"
  ctx.fillRect(w - 38 - barW + barW * (1 - p2EnergyRatio), topY + 44, barW * p2EnergyRatio, 12)
}
...
export function drawControlsInfo(ctx, canvas) {
  ...
  ctx.fillRect(18, h - 120, 280, 92)

  ctx.strokeStyle = "rgba(255,255,255,0.18)"
  ctx.strokeRect(18, h - 120, 280, 92)

  ctx.fillStyle = "#ffffff"
  ctx.font = "700 16px Arial"
  ctx.textAlign = "left"
  ctx.fillText("Controls", 32, h - 94)

  ctx.font = "14px Arial"
  ctx.fillStyle = "rgba(255,255,255,0.82)"
  ctx.fillText("Move / Jump / Light / Heavy / Special / Ultimate", 32, h - 70)
  ctx.fillText("Training: hitboxes, debug input, system toggles", 32, h - 48)
  ctx.restore()
}
...
export function drawTrainingOverlay(ctx, canvas, info = {}) {
  ...
  ctx.fillRect(16, 16, 270, 176)

  ctx.strokeStyle = "rgba(255,255,255,0.16)"
  ctx.strokeRect(16, 16, 270, 176)
  ...
  lines.forEach((line, i) => {
    ctx.fillText(line, 28, 40 + i * 18)
  })

  if (p1Inputs) {
    ctx.fillStyle = "#7fd3ff"
    ctx.fillText(`P1: ${p1Inputs}`, 28, 148)
  }

  if (p2Inputs) {
    ctx.fillStyle = "#ff9f9f"
    ctx.fillText(`P2: ${p2Inputs}`, 28, 166)
  }

  if (Array.isArray(info.history) && info.history.length && w >= 980) {
    ctx.fillStyle = "rgba(255,255,255,0.75)"
    ctx.fillText(`Last: ${info.history[0]?.display || "Neutral"}`, 28, 184)
  }
...
export function drawRoundBreak(ctx, canvas, winnerText = "ROUND BREAK") {
  ...
  drawCenteredText(ctx, winnerText, w / 2, h / 2 - 10, {
    font: "800 42px Arial",
    fill: "#ffffff"
  })

  drawSubText(ctx, "Prepare for the next round", w / 2, h / 2 + 42, {
    font: "18px Arial"
  })
  ctx.restore()
}
...
export function drawMatchEnd(ctx, canvas, winnerText = "MATCH OVER") {
  ...
  drawCenteredText(ctx, winnerText, w / 2, h / 2 - 18, {
    font: "900 46px Arial",
    fill: "#ffffff"
  })

  drawSubText(ctx, "Click to return to the title screen", w / 2, h / 2 + 42, {
    font: "18px Arial",
    fill: "rgba(220,230,255,0.80)"
  })
  ctx.restore()
}